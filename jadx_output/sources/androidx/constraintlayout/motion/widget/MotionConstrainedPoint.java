...
class MotionConstrainedPoint implements Comparable<MotionConstrainedPoint> {
    ...
    void different(MotionConstrainedPoint points, boolean[] mask, String[] custom) {
        ...
        mask[1] = mask[1] | diff(this.f58x, points.f58x);
        mask[2] = mask[2] | diff(this.f59y, points.f59y);
    ...
    void fillStandard(double[] data, int[] toUse) {
        int i = 0;
        float[] fArr = {this.position, this.f58x, this.f59y, this.width, this.height, this.alpha, this.elevation, this.rotation, this.rotationX, this.rotationY, this.scaleX, this.scaleY, this.mPivotX, this.mPivotY, this.translationX, this.translationY, this.translationZ, this.mPathRotate};
        for (int i2 : toUse) {
            if (i2 < 18) {
                data[i] = fArr[r4];
                i++;
