...
public class BCElGamalPublicKey implements ElGamalPublicKey, DHPublicKey {
    ...
    BCElGamalPublicKey(DHPublicKeySpec dHPublicKeySpec) {
        this.f4392y = dHPublicKeySpec.getY();
    ...
    BCElGamalPublicKey(SubjectPublicKeyInfo subjectPublicKeyInfo) {
        ...
        try {
            this.f4392y = ((ASN1Integer) subjectPublicKeyInfo.parsePublicKey()).getValue();
    ...
    BCElGamalPublicKey(ElGamalPublicKeyParameters elGamalPublicKeyParameters) {
        this.f4392y = elGamalPublicKeyParameters.getY();
    ...
    BCElGamalPublicKey(ElGamalPublicKey elGamalPublicKey) {
        this.f4392y = elGamalPublicKey.getY();
        this.elSpec = elGamalPublicKey.getParameters();
    }

    BCElGamalPublicKey(ElGamalPublicKeySpec elGamalPublicKeySpec) {
        this.f4392y = elGamalPublicKeySpec.getY();
    ...
    public byte[] getEncoded() {
        try {
            return new SubjectPublicKeyInfo(new AlgorithmIdentifier(OIWObjectIdentifiers.elGamalAlgorithm, new ElGamalParameter(this.elSpec.getP(), this.elSpec.getG())), new ASN1Integer(this.f4392y)).getEncoded(ASN1Encoding.DER);
        } catch (IOException unused) {
            return null;
        }
    }
