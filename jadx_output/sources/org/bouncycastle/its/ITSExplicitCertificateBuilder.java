...
public class ITSExplicitCertificateBuilder extends ITSCertificateBuilder {
    ...
    public ITSCertificate build(CertificateId certificateId, ITSPublicVerificationKey iTSPublicVerificationKey, ITSPublicEncryptionKey iTSPublicEncryptionKey) {
        ...
        VerificationKeyIndicator verificationKeyIndicator = this.signer.isForSelfSigning() ? toBeSignedCertificateCreateToBeSignedCertificate.getVerificationKeyIndicator() : this.signer.getAssociatedCertificate().toASN1Structure().getCertificateBase().getToBeSignedCertificate().getVerificationKeyIndicator();
        OutputStream outputStream = this.signer.getOutputStream();
        try {
            outputStream.write(OEREncoder.toByteArray(toBeSignedCertificateCreateToBeSignedCertificate, IEEE1609dot2.tbsCertificate));
            outputStream.close();
            ...
            builder2.setToBeSignedCertificate(toBeSignedCertificateCreateToBeSignedCertificate);
