...
public class LMSPublicKeyParameters extends LMSKeyParameters implements LMSContextBasedVerifier {
    ...
    private final byte[] f4633I;

    /* JADX INFO: renamed from: T1 */
    private final byte[] f4634T1;
    ...
    public LMSPublicKeyParameters(LMSigParameters lMSigParameters, LMOtsParameters lMOtsParameters, byte[] bArr, byte[] bArr2) {
        ...
        this.f4633I = Arrays.clone(bArr2);
        this.f4634T1 = Arrays.clone(bArr);
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (this.parameterSet.equals(lMSPublicKeyParameters.parameterSet) && this.lmOtsType.equals(lMSPublicKeyParameters.lmOtsType) && Arrays.areEqual(this.f4633I, lMSPublicKeyParameters.f4633I)) {
            return Arrays.areEqual(this.f4634T1, lMSPublicKeyParameters.f4634T1);
        }
        return false;
    }
    ...
    LMSContext generateOtsContext(LMSSignature lMSSignature) {
        ...
        if (lMSSignature.getOtsSignature().getType().getType() == type) {
            return new LMOtsPublicKey(LMOtsParameters.getParametersForType(type), this.f4633I, lMSSignature.getQ(), null).createOtsContext(lMSSignature);
        }
    ...
    public byte[] getI() {
        return Arrays.clone(this.f4633I);
    }
    ...
    public byte[] getT1() {
        return Arrays.clone(this.f4634T1);
    }

    public int hashCode() {
        return (((((this.parameterSet.hashCode() * 31) + this.lmOtsType.hashCode()) * 31) + Arrays.hashCode(this.f4633I)) * 31) + Arrays.hashCode(this.f4634T1);
    }

    boolean matchesT1(byte[] bArr) {
        return Arrays.constantTimeAreEqual(this.f4634T1, bArr);
    }
    ...
    byte[] toByteArray() {
        return Composer.compose().u32str(this.parameterSet.getType()).u32str(this.lmOtsType.getType()).bytes(this.f4633I).bytes(this.f4634T1).build();
    }
