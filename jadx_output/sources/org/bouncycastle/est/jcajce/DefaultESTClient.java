...
class DefaultESTClient implements ESTClient {
    ...
    private static byte[] CRLF = {Ascii.f384CR, 10};
    ...
    public ESTResponse performRequest(ESTRequest eSTRequest) throws IOException {
        ...
        try {
            ...
            OutputStream printingOutputStream = (setAsKeySet.contains(AgentOptions.OUTPUT) || setAsKeySet.contains("all")) ? new PrintingOutputStream(sourceMakeSource.getOutputStream()) : sourceMakeSource.getOutputStream();
            ...
            if (!eSTRequest.getHeaders().containsKey(HttpHeaders.CONNECTION)) {
                eSTRequestBuilder.addHeader(HttpHeaders.CONNECTION, "close");
            }
            URL url = eSTRequest.getURL();
            eSTRequestBuilder.setHeader(HttpHeaders.HOST, url.getPort() > -1 ? String.format("%s:%d", url.getHost(), Integer.valueOf(url.getPort())) : url.getHost());
            ESTRequest eSTRequestBuild = eSTRequestBuilder.build();
            writeLine(printingOutputStream, eSTRequestBuild.getMethod() + StringUtils.SPACE + str + " HTTP/1.1");
        ...
        if (eSTResponse.getStatusCode() < 300 || eSTResponse.getStatusCode() > 399) {
            ...
            switch (eSTResponse.getStatusCode()) {
                ...
                case 307:
                    ...
                    if (header.startsWith(ProxyConfig.MATCH_HTTP)) {
