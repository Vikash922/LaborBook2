...
public class JcaPublicKeyConverter {
    ...
    private static ECCurve convertCurve(EllipticCurve ellipticCurve, BigInteger bigInteger, int i) {
        ...
        if (field instanceof ECFieldFp) {
            return new ECCurve.C4651Fp(((ECFieldFp) field).getP(), a2, b, bigInteger, BigInteger.valueOf(i));
        }
    ...
    private ECParameterSpec getParams(ECDSAPublicKey eCDSAPublicKey) {
        ...
        ECCurve.C4651Fp c4651Fp = new ECCurve.C4651Fp(eCDSAPublicKey.getPrimeModulusP(), eCDSAPublicKey.getFirstCoefA(), eCDSAPublicKey.getSecondCoefB(), eCDSAPublicKey.getOrderOfBasePointR(), eCDSAPublicKey.getCofactorF());
        ECPoint eCPointDecodePoint = c4651Fp.decodePoint(eCDSAPublicKey.getBasePointG());
        return new ECParameterSpec(convertCurve(c4651Fp), new java.security.spec.ECPoint(eCPointDecodePoint.getAffineXCoord().toBigInteger(), eCPointDecodePoint.getAffineYCoord().toBigInteger()), eCDSAPublicKey.getOrderOfBasePointR(), eCDSAPublicKey.getCofactorF().intValue());
    }
    ...
    private java.security.spec.ECPoint getPublicPoint(ECDSAPublicKey eCDSAPublicKey) {
        ...
        ECPoint.C4654Fp c4654Fp = (ECPoint.C4654Fp) new ECCurve.C4651Fp(eCDSAPublicKey.getPrimeModulusP(), eCDSAPublicKey.getFirstCoefA(), eCDSAPublicKey.getSecondCoefB(), eCDSAPublicKey.getOrderOfBasePointR(), eCDSAPublicKey.getCofactorF()).decodePoint(eCDSAPublicKey.getPublicPointY());
        return new java.security.spec.ECPoint(c4654Fp.getAffineXCoord().toBigInteger(), c4654Fp.getAffineYCoord().toBigInteger());
    }

    public PublicKey getKey(PublicKeyDataObject publicKeyDataObject) throws EACException, InvalidKeySpecException {
        if (publicKeyDataObject.getUsage().m2764on(EACObjectIdentifiers.id_TA_ECDSA)) {
