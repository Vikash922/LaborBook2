...
public class TypeUtils {
    ...
    public static <T> Typed<T> wrap(final Type type) {
        return new Typed<T>() { // from class: org.shadow.apache.commons.lang3.reflect.TypeUtils.1
            @Override // org.shadow.apache.commons.lang3.reflect.Typed
            public Type getType() {
                return type;
            }
        };
    ...
    private static String parameterizedTypeToString(ParameterizedType parameterizedType) {
        ...
        appendAllTo(sb.append(Typography.less), ", ", actualTypeArguments).append(Typography.greater);
        return sb.toString();
    }
    ...
    private static String wildcardTypeToString(WildcardType wildcardType) {
        ...
        if (lowerBounds.length > 1 || (lowerBounds.length == 1 && lowerBounds[0] != null)) {
            appendAllTo(sbAppend.append(" super "), " & ", lowerBounds);
        } else if (upperBounds.length > 1 || (upperBounds.length == 1 && !Object.class.equals(upperBounds[0]))) {
            appendAllTo(sbAppend.append(" extends "), " & ", upperBounds);
        }
