...
public final class DebugMetadataKt {
    ...
    public static final StackTraceElement getStackTraceElement(BaseContinuationImpl baseContinuationImpl) {
        ...
        checkDebugMetadataVersion(1, debugMetadataAnnotation.m2741v());
        int label = getLabel(baseContinuationImpl);
        int i = label < 0 ? -1 : debugMetadataAnnotation.m2737l()[label];
        String moduleName = ModuleNameRetriever.INSTANCE.getModuleName(baseContinuationImpl);
        return new StackTraceElement(moduleName == null ? debugMetadataAnnotation.m2734c() : moduleName + '/' + debugMetadataAnnotation.m2734c(), debugMetadataAnnotation.m2738m(), debugMetadataAnnotation.m2735f(), i);
    }
    ...
    public static final String[] getSpilledVariableFieldMapping(BaseContinuationImpl baseContinuationImpl) {
        ...
        int label = getLabel(baseContinuationImpl);
        int[] iArrM2736i = debugMetadataAnnotation.m2736i();
        int length = iArrM2736i.length;
        for (int i = 0; i < length; i++) {
            if (iArrM2736i[i] == label) {
                arrayList.add(debugMetadataAnnotation.m2740s()[i]);
                arrayList.add(debugMetadataAnnotation.m2739n()[i]);
            }
        }
