...
class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        ...
        try {
            ...
            this.logger.m462d("Requesting settings from " + this.url);
            this.logger.m468v("Settings query params were: " + queryParamsFor);
            return handleResponse(httpGetRequestApplyHeadersTo.execute());
        } catch (IOException e) {
            this.logger.m465e("Settings request failed.", e);
            return null;
        }
    }
    ...
    JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        this.logger.m468v("Settings response code was: " + iCode);
        if (requestWasSuccessful(iCode)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        this.logger.m464e("Settings request failed; (status: " + iCode + ") from " + this.url);
        return null;
    }
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            this.logger.m471w("Failed to parse settings JSON from " + this.url, e);
            this.logger.m470w("Settings response " + str);
            return null;
        }
    }
