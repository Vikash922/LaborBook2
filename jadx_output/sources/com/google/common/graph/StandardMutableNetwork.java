...
final class StandardMutableNetwork<N, E> extends StandardNetwork<N, E> implements MutableNetwork<N, E> {
    ...
    public boolean addEdge(N n, N n2, E e) {
        ...
        if (containsEdge(e)) {
            ...
            EndpointPair endpointPairM400of = EndpointPair.m400of(this, n, n2);
            Preconditions.checkArgument(endpointPairIncidentNodes.equals(endpointPairM400of), "Edge %s already exists between the following nodes: %s, so it cannot be reused to connect the following nodes: %s.", e, endpointPairIncidentNodes, endpointPairM400of);
            return false;
        }
    ...
    private NetworkConnections<N, E> newConnections() {
        if (isDirected()) {
            if (allowsParallelEdges()) {
                return DirectedMultiNetworkConnections.m397of();
            }
            return DirectedNetworkConnections.m398of();
        }
        if (allowsParallelEdges()) {
            return UndirectedMultiNetworkConnections.m403of();
        }
        return UndirectedNetworkConnections.m404of();
    }
}
