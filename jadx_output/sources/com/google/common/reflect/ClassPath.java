...
public final class ClassPath {
    ...
    private static final Splitter CLASS_PATH_ATTRIBUTE_SEPARATOR = Splitter.m267on(StringUtils.SPACE).omitEmptyStrings();
    ...
    public ImmutableSet<ClassInfo> getTopLevelClassesRecursive(String str) {
        Preconditions.checkNotNull(str);
        String string = new StringBuilder(String.valueOf(str).length() + 1).append(str).append(ClassUtils.PACKAGE_SEPARATOR_CHAR).toString();
        ImmutableSet.Builder builder = ImmutableSet.builder();
        UnmodifiableIterator<ClassInfo> it = getTopLevelClasses().iterator();
        while (it.hasNext()) {
            ClassInfo next = it.next();
    ...
    public static class ResourceInfo {
        ...
        static ResourceInfo m436of(File file, String str, ClassLoader classLoader) {
        ...
        private void scanJarFile(JarFile jarFile, ImmutableSet.Builder<ResourceInfo> builder) {
            ...
            while (enumerationEntries.hasMoreElements()) {
                ...
                if (!jarEntryNextElement.isDirectory() && !jarEntryNextElement.getName().equals("META-INF/MANIFEST.MF")) {
                    builder.add(ResourceInfo.m436of(new File(jarFile.getName()), jarEntryNextElement.getName(), this.classloader));
                }
            }
        }
            ...
            for (File file2 : fileArrListFiles) {
                ...
                if (file2.isDirectory()) {
                    ...
                    if (!strConcat.equals("META-INF/MANIFEST.MF")) {
                        builder.add(ResourceInfo.m436of(file2, strConcat, this.classloader));
                    }
                }
            }
        }
    ...
    private static ImmutableList<URL> getClassLoaderUrls(ClassLoader classLoader) {
        ...
        return ImmutableList.m286of();
    }

    static ImmutableList<URL> parseJavaClassPath() {
        ImmutableList.Builder builder = ImmutableList.builder();
        for (String str : Splitter.m267on(StandardSystemProperty.PATH_SEPARATOR.value()).split(StandardSystemProperty.JAVA_CLASS_PATH.value())) {
            try {
                try {
    ...
    static String getClassName(String str) {
        return str.substring(0, str.length() - CLASS_FILE_NAME_EXTENSION.length()).replace('/', ClassUtils.PACKAGE_SEPARATOR_CHAR);
    }
