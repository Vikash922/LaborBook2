...
public class Jbig2SegmentReader {
    ...
    private RandomAccessFileOrArray f2854ra;
    ...
    public static class Jbig2Page {
        ...
        private final Jbig2SegmentReader f2855sr;
        ...
        public Jbig2Page(int i, Jbig2SegmentReader jbig2SegmentReader) {
            this.page = i;
            this.f2855sr = jbig2SegmentReader;
        }
    ...
    public Jbig2SegmentReader(RandomAccessFileOrArray randomAccessFileOrArray) {
        this.f2854ra = randomAccessFileOrArray;
    }
    ...
    public void read() throws IOException {
        ...
        if (this.sequential) {
            ...
            } while (this.f2854ra.getPosition() < this.f2854ra.length());
            return;
        }
        do {
            header = readHeader();
    ...
    void readSegment(Jbig2Segment jbig2Segment) throws IOException {
        int position = (int) this.f2854ra.getPosition();
        if (jbig2Segment.dataLength == BodyPartID.bodyIdMax) {
            return;
        }
        byte[] bArr = new byte[(int) jbig2Segment.dataLength];
        this.f2854ra.read(bArr);
        jbig2Segment.data = bArr;
        if (jbig2Segment.type == 48) {
            int position2 = (int) this.f2854ra.getPosition();
            this.f2854ra.seek(position);
            int i = this.f2854ra.readInt();
            int i2 = this.f2854ra.readInt();
            this.f2854ra.seek(position2);
            Jbig2Page jbig2Page = this.pages.get(Integer.valueOf(jbig2Segment.page));
            if (jbig2Page == null) {
                throw new com.itextpdf.p017io.exceptions.IOException("Referring to widht or height of a page we haven't seen yet: {0}").setMessageParams(Integer.valueOf(jbig2Segment.page));
            }
    ...
    Jbig2Segment readHeader() throws IOException {
        ...
        if (i5 == 7) {
            ...
            if (i5 == 5 || i5 == 6) {
                throw new com.itextpdf.p017io.exceptions.IOException("Count of referred-to segments has forbidden value in the header for segment {0} starting at {1}").setMessageParams(Integer.valueOf(i2), Integer.valueOf(position));
            }
            zArr = null;
        }
        ...
        for (int i12 = 1; i12 <= i5; i12++) {
            if (i2 <= 256) {
                iArr[i12] = this.f2854ra.read();
            } else if (i2 <= 65536) {
                iArr[i12] = this.f2854ra.readUnsignedShort();
            } else {
                iArr[i12] = (int) this.f2854ra.readUnsignedInt();
            }
        }
        jbig2Segment.referredToSegmentNumbers = iArr;
        int position2 = ((int) this.f2854ra.getPosition()) - position;
        if (z) {
            i = this.f2854ra.readInt();
        } else {
            i = this.f2854ra.read();
        }
        if (i < 0) {
            throw new com.itextpdf.p017io.exceptions.IOException("Page {0} is invalid for segment {1} starting at {2}").setMessageParams(Integer.valueOf(i), Integer.valueOf(i2), Integer.valueOf(position));
        }
        jbig2Segment.page = i;
        ...
        jbig2Segment.dataLength = this.f2854ra.readUnsignedInt();
        int position3 = (int) this.f2854ra.getPosition();
        this.f2854ra.seek(position);
    ...
    void readFileHeader() throws IOException {
        this.f2854ra.seek(0L);
        byte[] bArr = new byte[8];
        this.f2854ra.read(bArr);
        byte[] bArr2 = {-105, 74, 66, 50, Ascii.f384CR, 10, Ascii.SUB, 10};
        for (int i = 0; i < 8; i++) {
            if (bArr[i] != bArr2[i]) {
                throw new com.itextpdf.p017io.exceptions.IOException("File header idstring is not good at byte {0}").setMessageParams(Integer.valueOf(i));
            }
        }
        int i2 = this.f2854ra.read();
        this.sequential = (i2 & 1) == 1;
        boolean z = (i2 & 2) == 0;
        this.number_of_pages_known = z;
        if ((i2 & 252) != 0) {
            throw new com.itextpdf.p017io.exceptions.IOException("File header flags bits from 2 to 7 should be 0, some not");
        }
        if (z) {
            this.number_of_pages = this.f2854ra.readInt();
        }
    }
