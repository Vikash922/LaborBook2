...
public class BezierCurve implements IShape {
    ...
    private void recursiveApproximation(double d, double d2, double d3, double d4, double d5, double d6, double d7, double d8, List<Point> list) {
        double d9 = (d + d3) / 2.0d;
        ...
        double d21 = d7 - d;
        double d22 = d8 - d2;
        double dAbs = Math.abs(((d3 - d7) * d22) - ((d4 - d8) * d21));
        double dAbs2 = Math.abs(((d5 - d7) * d22) - ((d6 - d8) * d21));
        double d23 = curveCollinearityEpsilon;
        if (dAbs > d23 || dAbs2 > d23) {
            double d24 = dAbs + dAbs2;
            if (d24 * d24 <= distanceToleranceSquare * ((d21 * d21) + (d22 * d22))) {
                list.add(new Point(d19, d20));
                return;
            }
