...
public final class Util {
    ...
    public static int getBitmapByteSize(Bitmap bitmap) {
        if (bitmap.isRecycled()) {
            throw new IllegalStateException("Cannot obtain size for recycled Bitmap: " + bitmap + "[" + bitmap.getWidth() + SvgConstants.Attributes.f3330X + bitmap.getHeight() + "] " + bitmap.getConfig());
        }
        try {
    ...
    public static int getBytesPerPixel(Bitmap.Config config) {
        ...
        int i = C11901.$SwitchMap$android$graphics$Bitmap$Config[config.ordinal()];
        if (i == 1) {
            return 1;
        }
        if (i == 2 || i == 3) {
