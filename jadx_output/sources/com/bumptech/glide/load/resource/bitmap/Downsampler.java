...
public final class Downsampler {
        ...
        if (shouldUsePool(imageType)) {
            if (i7 < 0 || i8 < 0 || !z2) {
                ...
                if (Log.isLoggable(TAG, 2)) {
                    Log.v(TAG, "Calculated target [" + iRound + SvgConstants.Attributes.f3330X + iRound2 + "] for source [" + i7 + SvgConstants.Attributes.f3330X + i8 + "], sampleSize: " + i10 + ", targetDensity: " + options.inTargetDensity + ", density: " + options.inDensity + ", density multiplier: " + f);
                }
            } else {
                iRound = i4;
                iRound2 = i5;
    ...
    private static void calculateScaling(ImageHeaderParser.ImageType imageType, ImageReader imageReader, DecodeCallbacks decodeCallbacks, BitmapPool bitmapPool, DownsampleStrategy downsampleStrategy, int i, int i2, int i3, int i4, int i5, BitmapFactory.Options options) throws IOException {
        ...
        if (i2 <= 0 || i3 <= 0) {
            if (Log.isLoggable(TAG, 3)) {
                Log.d(TAG, "Unable to determine dimensions for: " + imageType + " with target [" + i4 + SvgConstants.Attributes.f3330X + i5 + "]");
                return;
            }
            return;
        }
        ...
        if (scaleFactor <= 0.0f) {
            throw new IllegalArgumentException("Cannot scale with factor: " + scaleFactor + " from: " + downsampleStrategy + ", source: [" + i2 + SvgConstants.Attributes.f3330X + i3 + "], target: [" + i4 + SvgConstants.Attributes.f3330X + i5 + "]");
        }
        ...
        if (Log.isLoggable(TAG, 2)) {
            Log.v(TAG, "Calculate scaling, source: [" + i2 + SvgConstants.Attributes.f3330X + i3 + "], degreesToRotate: " + i + ", target: [" + i4 + SvgConstants.Attributes.f3330X + i5 + "], power of two scaled: [" + iFloor + SvgConstants.Attributes.f3330X + iFloor2 + "], exact scale factor: " + scaleFactor + ", power of 2 sample size: " + iMax + ", adjusted scale factor: " + scaleFactor2 + ", target density: " + options.inTargetDensity + ", density: " + options.inDensity);
        }
    }
    ...
    */
    ...
    private static void logDecode(int i, int i2, String str, BitmapFactory.Options options, Bitmap bitmap, int i3, int i4, long j) {
        Log.v(TAG, "Decoded " + getBitmapString(bitmap) + " from [" + i + SvgConstants.Attributes.f3330X + i2 + "] " + str + " with inBitmap " + getInBitmapString(options) + " for [" + i3 + SvgConstants.Attributes.f3330X + i4 + "], sample size: " + options.inSampleSize + ", density: " + options.inDensity + ", target density: " + options.inTargetDensity + ", thread: " + Thread.currentThread().getName() + ", duration: " + LogTime.getElapsedMillis(j));
    }
    ...
    private static String getBitmapString(Bitmap bitmap) {
        ...
        return "[" + bitmap.getWidth() + SvgConstants.Attributes.f3330X + bitmap.getHeight() + "] " + bitmap.getConfig() + (" (" + bitmap.getAllocationByteCount() + ")");
    }
