.class final Lcom/itextpdf/barcodes/qrcode/MatrixUtil;
.super Ljava/lang/Object;
.source "MatrixUtil.java"


# static fields
.field private static final HORIZONTAL_SEPARATION_PATTERN:[[I

.field private static final POSITION_ADJUSTMENT_PATTERN:[[I

.field private static final POSITION_ADJUSTMENT_PATTERN_COORDINATE_TABLE:[[I

.field private static final POSITION_DETECTION_PATTERN:[[I

.field private static final TYPE_INFO_COORDINATES:[[I

.field private static final TYPE_INFO_MASK_PATTERN:I = 0x5412

.field private static final TYPE_INFO_POLY:I = 0x537

.field private static final VERSION_INFO_POLY:I = 0x1f25

.field private static final VERTICAL_SEPARATION_PATTERN:[[I


# direct methods
.method static constructor <clinit>()V
    .locals 45

    const/4 v0, 0x7

    .line 57
    new-array v1, v0, [I

    fill-array-data v1, :array_0

    new-array v2, v0, [I

    fill-array-data v2, :array_1

    new-array v3, v0, [I

    fill-array-data v3, :array_2

    new-array v4, v0, [I

    fill-array-data v4, :array_3

    new-array v5, v0, [I

    fill-array-data v5, :array_4

    new-array v6, v0, [I

    fill-array-data v6, :array_5

    new-array v7, v0, [I

    fill-array-data v7, :array_6

    filled-new-array/range {v1 .. v7}, [[I

    move-result-object v1

    sput-object v1, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->POSITION_DETECTION_PATTERN:[[I

    const/16 v1, 0x8

    .line 67
    new-array v2, v1, [I

    fill-array-data v2, :array_7

    filled-new-array {v2}, [[I

    move-result-object v2

    sput-object v2, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->HORIZONTAL_SEPARATION_PATTERN:[[I

    const/4 v2, 0x0

    .line 71
    filled-new-array {v2}, [I

    move-result-object v3

    filled-new-array {v2}, [I

    move-result-object v4

    filled-new-array {v2}, [I

    move-result-object v5

    filled-new-array {v2}, [I

    move-result-object v6

    filled-new-array {v2}, [I

    move-result-object v7

    filled-new-array {v2}, [I

    move-result-object v8

    filled-new-array {v2}, [I

    move-result-object v9

    filled-new-array/range {v3 .. v9}, [[I

    move-result-object v3

    sput-object v3, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->VERTICAL_SEPARATION_PATTERN:[[I

    const/4 v3, 0x1

    .line 75
    filled-new-array {v3, v3, v3, v3, v3}, [I

    move-result-object v4

    filled-new-array {v3, v2, v2, v2, v3}, [I

    move-result-object v5

    filled-new-array {v3, v2, v3, v2, v3}, [I

    move-result-object v6

    filled-new-array {v3, v2, v2, v2, v3}, [I

    move-result-object v7

    filled-new-array {v3, v3, v3, v3, v3}, [I

    move-result-object v8

    filled-new-array {v4, v5, v6, v7, v8}, [[I

    move-result-object v4

    sput-object v4, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->POSITION_ADJUSTMENT_PATTERN:[[I

    .line 84
    new-array v4, v0, [I

    move-object v5, v4

    fill-array-data v4, :array_8

    new-array v4, v0, [I

    move-object v6, v4

    fill-array-data v4, :array_9

    new-array v4, v0, [I

    move-object v7, v4

    fill-array-data v4, :array_a

    new-array v4, v0, [I

    move-object v8, v4

    fill-array-data v4, :array_b

    new-array v4, v0, [I

    move-object v9, v4

    fill-array-data v4, :array_c

    new-array v4, v0, [I

    move-object v10, v4

    fill-array-data v4, :array_d

    new-array v4, v0, [I

    move-object v11, v4

    fill-array-data v4, :array_e

    new-array v4, v0, [I

    move-object v12, v4

    fill-array-data v4, :array_f

    new-array v4, v0, [I

    move-object v13, v4

    fill-array-data v4, :array_10

    new-array v4, v0, [I

    move-object v14, v4

    fill-array-data v4, :array_11

    new-array v4, v0, [I

    move-object v15, v4

    fill-array-data v4, :array_12

    new-array v4, v0, [I

    move-object/from16 v16, v4

    fill-array-data v4, :array_13

    new-array v4, v0, [I

    move-object/from16 v17, v4

    fill-array-data v4, :array_14

    new-array v4, v0, [I

    move-object/from16 v18, v4

    fill-array-data v4, :array_15

    new-array v4, v0, [I

    move-object/from16 v19, v4

    fill-array-data v4, :array_16

    new-array v4, v0, [I

    move-object/from16 v20, v4

    fill-array-data v4, :array_17

    new-array v4, v0, [I

    move-object/from16 v21, v4

    fill-array-data v4, :array_18

    new-array v4, v0, [I

    move-object/from16 v22, v4

    fill-array-data v4, :array_19

    new-array v4, v0, [I

    move-object/from16 v23, v4

    fill-array-data v4, :array_1a

    new-array v4, v0, [I

    move-object/from16 v24, v4

    fill-array-data v4, :array_1b

    new-array v4, v0, [I

    move-object/from16 v25, v4

    fill-array-data v4, :array_1c

    new-array v4, v0, [I

    move-object/from16 v26, v4

    fill-array-data v4, :array_1d

    new-array v4, v0, [I

    move-object/from16 v27, v4

    fill-array-data v4, :array_1e

    new-array v4, v0, [I

    move-object/from16 v28, v4

    fill-array-data v4, :array_1f

    new-array v4, v0, [I

    move-object/from16 v29, v4

    fill-array-data v4, :array_20

    new-array v4, v0, [I

    move-object/from16 v30, v4

    fill-array-data v4, :array_21

    new-array v4, v0, [I

    move-object/from16 v31, v4

    fill-array-data v4, :array_22

    new-array v4, v0, [I

    move-object/from16 v32, v4

    fill-array-data v4, :array_23

    new-array v4, v0, [I

    move-object/from16 v33, v4

    fill-array-data v4, :array_24

    new-array v4, v0, [I

    move-object/from16 v34, v4

    fill-array-data v4, :array_25

    new-array v4, v0, [I

    move-object/from16 v35, v4

    fill-array-data v4, :array_26

    new-array v4, v0, [I

    move-object/from16 v36, v4

    fill-array-data v4, :array_27

    new-array v4, v0, [I

    move-object/from16 v37, v4

    fill-array-data v4, :array_28

    new-array v4, v0, [I

    move-object/from16 v38, v4

    fill-array-data v4, :array_29

    new-array v4, v0, [I

    move-object/from16 v39, v4

    fill-array-data v4, :array_2a

    new-array v4, v0, [I

    move-object/from16 v40, v4

    fill-array-data v4, :array_2b

    new-array v4, v0, [I

    move-object/from16 v41, v4

    fill-array-data v4, :array_2c

    new-array v4, v0, [I

    move-object/from16 v42, v4

    fill-array-data v4, :array_2d

    new-array v4, v0, [I

    move-object/from16 v43, v4

    fill-array-data v4, :array_2e

    new-array v4, v0, [I

    move-object/from16 v44, v4

    fill-array-data v4, :array_2f

    filled-new-array/range {v5 .. v44}, [[I

    move-result-object v4

    sput-object v4, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->POSITION_ADJUSTMENT_PATTERN_COORDINATE_TABLE:[[I

    .line 208
    filled-new-array {v1, v2}, [I

    move-result-object v5

    filled-new-array {v1, v3}, [I

    move-result-object v6

    const/4 v4, 0x2

    filled-new-array {v1, v4}, [I

    move-result-object v7

    const/4 v4, 0x3

    filled-new-array {v1, v4}, [I

    move-result-object v8

    const/4 v4, 0x4

    filled-new-array {v1, v4}, [I

    move-result-object v9

    const/4 v4, 0x5

    filled-new-array {v1, v4}, [I

    move-result-object v10

    filled-new-array {v1, v0}, [I

    move-result-object v11

    filled-new-array {v1, v1}, [I

    move-result-object v12

    filled-new-array {v0, v1}, [I

    move-result-object v13

    const/4 v0, 0x5

    filled-new-array {v0, v1}, [I

    move-result-object v14

    const/4 v0, 0x4

    filled-new-array {v0, v1}, [I

    move-result-object v15

    const/4 v0, 0x3

    filled-new-array {v0, v1}, [I

    move-result-object v16

    const/4 v0, 0x2

    filled-new-array {v0, v1}, [I

    move-result-object v17

    filled-new-array {v3, v1}, [I

    move-result-object v18

    filled-new-array {v2, v1}, [I

    move-result-object v19

    filled-new-array/range {v5 .. v19}, [[I

    move-result-object v0

    sput-object v0, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->TYPE_INFO_COORDINATES:[[I

    return-void

    nop

    :array_0
    .array-data 4
        0x1
        0x1
        0x1
        0x1
        0x1
        0x1
        0x1
    .end array-data

    :array_1
    .array-data 4
        0x1
        0x0
        0x0
        0x0
        0x0
        0x0
        0x1
    .end array-data

    :array_2
    .array-data 4
        0x1
        0x0
        0x1
        0x1
        0x1
        0x0
        0x1
    .end array-data

    :array_3
    .array-data 4
        0x1
        0x0
        0x1
        0x1
        0x1
        0x0
        0x1
    .end array-data

    :array_4
    .array-data 4
        0x1
        0x0
        0x1
        0x1
        0x1
        0x0
        0x1
    .end array-data

    :array_5
    .array-data 4
        0x1
        0x0
        0x0
        0x0
        0x0
        0x0
        0x1
    .end array-data

    :array_6
    .array-data 4
        0x1
        0x1
        0x1
        0x1
        0x1
        0x1
        0x1
    .end array-data

    :array_7
    .array-data 4
        0x0
        0x0
        0x0
        0x0
        0x0
        0x0
        0x0
        0x0
    .end array-data

    :array_8
    .array-data 4
        -0x1
        -0x1
        -0x1
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_9
    .array-data 4
        0x6
        0x12
        -0x1
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_a
    .array-data 4
        0x6
        0x16
        -0x1
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_b
    .array-data 4
        0x6
        0x1a
        -0x1
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_c
    .array-data 4
        0x6
        0x1e
        -0x1
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_d
    .array-data 4
        0x6
        0x22
        -0x1
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_e
    .array-data 4
        0x6
        0x16
        0x26
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_f
    .array-data 4
        0x6
        0x18
        0x2a
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_10
    .array-data 4
        0x6
        0x1a
        0x2e
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_11
    .array-data 4
        0x6
        0x1c
        0x32
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_12
    .array-data 4
        0x6
        0x1e
        0x36
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_13
    .array-data 4
        0x6
        0x20
        0x3a
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_14
    .array-data 4
        0x6
        0x22
        0x3e
        -0x1
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_15
    .array-data 4
        0x6
        0x1a
        0x2e
        0x42
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_16
    .array-data 4
        0x6
        0x1a
        0x30
        0x46
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_17
    .array-data 4
        0x6
        0x1a
        0x32
        0x4a
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_18
    .array-data 4
        0x6
        0x1e
        0x36
        0x4e
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_19
    .array-data 4
        0x6
        0x1e
        0x38
        0x52
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_1a
    .array-data 4
        0x6
        0x1e
        0x3a
        0x56
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_1b
    .array-data 4
        0x6
        0x22
        0x3e
        0x5a
        -0x1
        -0x1
        -0x1
    .end array-data

    :array_1c
    .array-data 4
        0x6
        0x1c
        0x32
        0x48
        0x5e
        -0x1
        -0x1
    .end array-data

    :array_1d
    .array-data 4
        0x6
        0x1a
        0x32
        0x4a
        0x62
        -0x1
        -0x1
    .end array-data

    :array_1e
    .array-data 4
        0x6
        0x1e
        0x36
        0x4e
        0x66
        -0x1
        -0x1
    .end array-data

    :array_1f
    .array-data 4
        0x6
        0x1c
        0x36
        0x50
        0x6a
        -0x1
        -0x1
    .end array-data

    :array_20
    .array-data 4
        0x6
        0x20
        0x3a
        0x54
        0x6e
        -0x1
        -0x1
    .end array-data

    :array_21
    .array-data 4
        0x6
        0x1e
        0x3a
        0x56
        0x72
        -0x1
        -0x1
    .end array-data

    :array_22
    .array-data 4
        0x6
        0x22
        0x3e
        0x5a
        0x76
        -0x1
        -0x1
    .end array-data

    :array_23
    .array-data 4
        0x6
        0x1a
        0x32
        0x4a
        0x62
        0x7a
        -0x1
    .end array-data

    :array_24
    .array-data 4
        0x6
        0x1e
        0x36
        0x4e
        0x66
        0x7e
        -0x1
    .end array-data

    :array_25
    .array-data 4
        0x6
        0x1a
        0x34
        0x4e
        0x68
        0x82
        -0x1
    .end array-data

    :array_26
    .array-data 4
        0x6
        0x1e
        0x38
        0x52
        0x6c
        0x86
        -0x1
    .end array-data

    :array_27
    .array-data 4
        0x6
        0x22
        0x3c
        0x56
        0x70
        0x8a
        -0x1
    .end array-data

    :array_28
    .array-data 4
        0x6
        0x1e
        0x3a
        0x56
        0x72
        0x8e
        -0x1
    .end array-data

    :array_29
    .array-data 4
        0x6
        0x22
        0x3e
        0x5a
        0x76
        0x92
        -0x1
    .end array-data

    :array_2a
    .array-data 4
        0x6
        0x1e
        0x36
        0x4e
        0x66
        0x7e
        0x96
    .end array-data

    :array_2b
    .array-data 4
        0x6
        0x18
        0x32
        0x4c
        0x66
        0x80
        0x9a
    .end array-data

    :array_2c
    .array-data 4
        0x6
        0x1c
        0x36
        0x50
        0x6a
        0x84
        0x9e
    .end array-data

    :array_2d
    .array-data 4
        0x6
        0x20
        0x3a
        0x54
        0x6e
        0x88
        0xa2
    .end array-data

    :array_2e
    .array-data 4
        0x6
        0x1a
        0x36
        0x52
        0x6e
        0x8a
        0xa6
    .end array-data

    :array_2f
    .array-data 4
        0x6
        0x1e
        0x3a
        0x56
        0x72
        0x8e
        0xaa
    .end array-data
.end method

.method private constructor <init>()V
    .locals 0

    .line 54
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method

.method public static buildMatrix(Lcom/itextpdf/barcodes/qrcode/BitVector;Lcom/itextpdf/barcodes/qrcode/ErrorCorrectionLevel;IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 0
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    .line 254
    invoke-static {p4}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->clearMatrix(Lcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 255
    invoke-static {p2, p4}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedBasicPatterns(ILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 257
    invoke-static {p1, p3, p4}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedTypeInfo(Lcom/itextpdf/barcodes/qrcode/ErrorCorrectionLevel;ILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 259
    invoke-static {p2, p4}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->maybeEmbedVersionInfo(ILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 261
    invoke-static {p0, p3, p4}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedDataBits(Lcom/itextpdf/barcodes/qrcode/BitVector;ILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    return-void
.end method

.method public static calculateBCHCode(II)I
    .locals 2

    .line 481
    invoke-static {p1}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->findMSBSet(I)I

    move-result v0

    add-int/lit8 v1, v0, -0x1

    shl-int/2addr p0, v1

    .line 484
    :goto_0
    invoke-static {p0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->findMSBSet(I)I

    move-result v1

    if-lt v1, v0, :cond_0

    .line 485
    invoke-static {p0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->findMSBSet(I)I

    move-result v1

    sub-int/2addr v1, v0

    shl-int v1, p1, v1

    xor-int/2addr p0, v1

    goto :goto_0

    :cond_0
    return p0
.end method

.method public static clearMatrix(Lcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 1

    const/4 v0, -0x1

    .line 239
    invoke-virtual {p0, v0}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->clear(B)V

    return-void
.end method

.method public static embedBasicPatterns(ILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 0
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    .line 277
    invoke-static {p1}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedPositionDetectionPatternsAndSeparators(Lcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 279
    invoke-static {p1}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedDarkDotAtLeftBottomCorner(Lcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 282
    invoke-static {p0, p1}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->maybeEmbedPositionAdjustmentPatterns(ILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 284
    invoke-static {p1}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedTimingPatterns(Lcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    return-void
.end method

.method private static embedDarkDotAtLeftBottomCorner(Lcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 3
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    .line 586
    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getHeight()I

    move-result v0

    const/16 v1, 0x8

    sub-int/2addr v0, v1

    invoke-virtual {p0, v1, v0}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->get(II)B

    move-result v0

    if-eqz v0, :cond_0

    .line 589
    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getHeight()I

    move-result v0

    sub-int/2addr v0, v1

    const/4 v2, 0x1

    invoke-virtual {p0, v1, v0, v2}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    return-void

    .line 587
    :cond_0
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    invoke-direct {p0}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>()V

    throw p0
.end method

.method public static embedDataBits(Lcom/itextpdf/barcodes/qrcode/BitVector;ILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 10
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    .line 379
    invoke-virtual {p2}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getWidth()I

    move-result v0

    add-int/lit8 v0, v0, -0x1

    .line 380
    invoke-virtual {p2}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getHeight()I

    move-result v1

    add-int/lit8 v1, v1, -0x1

    const/4 v2, 0x0

    const/4 v3, -0x1

    move v4, v2

    move v5, v3

    :goto_0
    if-lez v0, :cond_6

    const/4 v6, 0x6

    if-ne v0, v6, :cond_0

    add-int/lit8 v0, v0, -0x1

    :cond_0
    :goto_1
    if-ltz v1, :cond_5

    .line 387
    invoke-virtual {p2}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getHeight()I

    move-result v6

    if-ge v1, v6, :cond_5

    move v6, v2

    :goto_2
    const/4 v7, 0x2

    if-ge v6, v7, :cond_4

    sub-int v7, v0, v6

    .line 392
    invoke-virtual {p2, v7, v1}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->get(II)B

    move-result v8

    invoke-static {v8}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->isEmpty(B)Z

    move-result v8

    if-nez v8, :cond_1

    goto :goto_4

    .line 396
    :cond_1
    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/BitVector;->size()I

    move-result v8

    if-ge v4, v8, :cond_2

    .line 397
    invoke-virtual {p0, v4}, Lcom/itextpdf/barcodes/qrcode/BitVector;->at(I)I

    move-result v8

    add-int/lit8 v4, v4, 0x1

    goto :goto_3

    :cond_2
    move v8, v2

    :goto_3
    if-eq p1, v3, :cond_3

    .line 408
    invoke-static {p1, v7, v1}, Lcom/itextpdf/barcodes/qrcode/MaskUtil;->getDataMaskBit(III)Z

    move-result v9

    if-eqz v9, :cond_3

    xor-int/lit8 v8, v8, 0x1

    .line 412
    :cond_3
    invoke-virtual {p2, v7, v1, v8}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    :goto_4
    add-int/lit8 v6, v6, 0x1

    goto :goto_2

    :cond_4
    add-int/2addr v1, v5

    goto :goto_1

    :cond_5
    neg-int v5, v5

    add-int/2addr v1, v5

    add-int/lit8 v0, v0, -0x2

    goto :goto_0

    .line 426
    :cond_6
    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/BitVector;->size()I

    move-result p1

    if-ne v4, p1, :cond_7

    return-void

    .line 427
    :cond_7
    new-instance p1, Lcom/itextpdf/barcodes/exceptions/WriterException;

    new-instance p2, Ljava/lang/StringBuilder;

    const-string v0, "Not all bits consumed: "

    invoke-direct {p2, v0}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p2, v4}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object p2

    const/16 v0, 0x2f

    invoke-virtual {p2, v0}, Ljava/lang/StringBuilder;->append(C)Ljava/lang/StringBuilder;

    move-result-object p2

    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/BitVector;->size()I

    move-result p0

    invoke-virtual {p2, p0}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object p0

    invoke-virtual {p0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, p0}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>(Ljava/lang/String;)V

    throw p1
.end method

.method private static embedHorizontalSeparationPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 5
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    .line 595
    sget-object v0, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->HORIZONTAL_SEPARATION_PATTERN:[[I

    const/4 v1, 0x0

    aget-object v2, v0, v1

    array-length v2, v2

    const/16 v3, 0x8

    if-ne v2, v3, :cond_2

    array-length v0, v0

    const/4 v2, 0x1

    if-ne v0, v2, :cond_2

    move v0, v1

    :goto_0
    if-ge v0, v3, :cond_1

    add-int v2, p0, v0

    .line 599
    invoke-virtual {p2, v2, p1}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->get(II)B

    move-result v4

    invoke-static {v4}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->isEmpty(B)Z

    move-result v4

    if-eqz v4, :cond_0

    .line 602
    sget-object v4, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->HORIZONTAL_SEPARATION_PATTERN:[[I

    aget-object v4, v4, v1

    aget v4, v4, v0

    invoke-virtual {p2, v2, p1, v4}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    add-int/lit8 v0, v0, 0x1

    goto :goto_0

    .line 600
    :cond_0
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    invoke-direct {p0}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>()V

    throw p0

    :cond_1
    return-void

    .line 596
    :cond_2
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    const-string p1, "Bad horizontal separation pattern"

    invoke-direct {p0, p1}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>(Ljava/lang/String;)V

    throw p0
.end method

.method private static embedPositionAdjustmentPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 7
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    .line 626
    sget-object v0, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->POSITION_ADJUSTMENT_PATTERN:[[I

    const/4 v1, 0x0

    aget-object v2, v0, v1

    array-length v2, v2

    const/4 v3, 0x5

    if-ne v2, v3, :cond_3

    array-length v0, v0

    if-ne v0, v3, :cond_3

    move v0, v1

    :goto_0
    if-ge v0, v3, :cond_2

    move v2, v1

    :goto_1
    if-ge v2, v3, :cond_1

    add-int v4, p0, v2

    add-int v5, p1, v0

    .line 631
    invoke-virtual {p2, v4, v5}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->get(II)B

    move-result v6

    invoke-static {v6}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->isEmpty(B)Z

    move-result v6

    if-eqz v6, :cond_0

    .line 634
    sget-object v6, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->POSITION_ADJUSTMENT_PATTERN:[[I

    aget-object v6, v6, v0

    aget v6, v6, v2

    invoke-virtual {p2, v4, v5, v6}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    add-int/lit8 v2, v2, 0x1

    goto :goto_1

    .line 632
    :cond_0
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    invoke-direct {p0}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>()V

    throw p0

    :cond_1
    add-int/lit8 v0, v0, 0x1

    goto :goto_0

    :cond_2
    return-void

    .line 627
    :cond_3
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    const-string p1, "Bad position adjustment"

    invoke-direct {p0, p1}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>(Ljava/lang/String;)V

    throw p0
.end method

.method private static embedPositionDetectionPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 7
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    .line 642
    sget-object v0, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->POSITION_DETECTION_PATTERN:[[I

    const/4 v1, 0x0

    aget-object v2, v0, v1

    array-length v2, v2

    const/4 v3, 0x7

    if-ne v2, v3, :cond_3

    array-length v0, v0

    if-ne v0, v3, :cond_3

    move v0, v1

    :goto_0
    if-ge v0, v3, :cond_2

    move v2, v1

    :goto_1
    if-ge v2, v3, :cond_1

    add-int v4, p0, v2

    add-int v5, p1, v0

    .line 647
    invoke-virtual {p2, v4, v5}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->get(II)B

    move-result v6

    invoke-static {v6}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->isEmpty(B)Z

    move-result v6

    if-eqz v6, :cond_0

    .line 650
    sget-object v6, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->POSITION_DETECTION_PATTERN:[[I

    aget-object v6, v6, v0

    aget v6, v6, v2

    invoke-virtual {p2, v4, v5, v6}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    add-int/lit8 v2, v2, 0x1

    goto :goto_1

    .line 648
    :cond_0
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    invoke-direct {p0}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>()V

    throw p0

    :cond_1
    add-int/lit8 v0, v0, 0x1

    goto :goto_0

    :cond_2
    return-void

    .line 643
    :cond_3
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    const-string p1, "Bad position detection pattern"

    invoke-direct {p0, p1}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>(Ljava/lang/String;)V

    throw p0
.end method

.method private static embedPositionDetectionPatternsAndSeparators(Lcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 4
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    .line 658
    sget-object v0, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->POSITION_DETECTION_PATTERN:[[I

    const/4 v1, 0x0

    aget-object v0, v0, v1

    array-length v0, v0

    .line 660
    invoke-static {v1, v1, p0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedPositionDetectionPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 662
    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getWidth()I

    move-result v2

    sub-int/2addr v2, v0

    invoke-static {v2, v1, p0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedPositionDetectionPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 664
    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getWidth()I

    move-result v2

    sub-int/2addr v2, v0

    invoke-static {v1, v2, p0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedPositionDetectionPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 667
    sget-object v0, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->HORIZONTAL_SEPARATION_PATTERN:[[I

    aget-object v0, v0, v1

    array-length v0, v0

    add-int/lit8 v2, v0, -0x1

    .line 669
    invoke-static {v1, v2, p0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedHorizontalSeparationPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 671
    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getWidth()I

    move-result v3

    sub-int/2addr v3, v0

    invoke-static {v3, v2, p0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedHorizontalSeparationPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 674
    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getWidth()I

    move-result v2

    sub-int/2addr v2, v0

    invoke-static {v1, v2, p0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedHorizontalSeparationPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 677
    sget-object v0, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->VERTICAL_SEPARATION_PATTERN:[[I

    array-length v0, v0

    .line 679
    invoke-static {v0, v1, p0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedVerticalSeparationPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 681
    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getHeight()I

    move-result v2

    sub-int/2addr v2, v0

    add-int/lit8 v2, v2, -0x1

    invoke-static {v2, v1, p0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedVerticalSeparationPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    .line 683
    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getHeight()I

    move-result v1

    sub-int/2addr v1, v0

    invoke-static {v0, v1, p0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedVerticalSeparationPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    return-void
.end method

.method private static embedTimingPatterns(Lcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 6
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    const/16 v0, 0x8

    move v1, v0

    .line 563
    :goto_0
    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getWidth()I

    move-result v2

    sub-int/2addr v2, v0

    if-ge v1, v2, :cond_4

    add-int/lit8 v2, v1, 0x1

    .line 564
    rem-int/lit8 v3, v2, 0x2

    const/4 v4, 0x6

    .line 567
    invoke-virtual {p0, v1, v4}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->get(II)B

    move-result v5

    invoke-static {v5}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->isValidValue(B)Z

    move-result v5

    if-eqz v5, :cond_3

    .line 570
    invoke-virtual {p0, v1, v4}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->get(II)B

    move-result v5

    invoke-static {v5}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->isEmpty(B)Z

    move-result v5

    if-eqz v5, :cond_0

    .line 571
    invoke-virtual {p0, v1, v4, v3}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    .line 575
    :cond_0
    invoke-virtual {p0, v4, v1}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->get(II)B

    move-result v5

    invoke-static {v5}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->isValidValue(B)Z

    move-result v5

    if-eqz v5, :cond_2

    .line 578
    invoke-virtual {p0, v4, v1}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->get(II)B

    move-result v5

    invoke-static {v5}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->isEmpty(B)Z

    move-result v5

    if-eqz v5, :cond_1

    .line 579
    invoke-virtual {p0, v4, v1, v3}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    :cond_1
    move v1, v2

    goto :goto_0

    .line 576
    :cond_2
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    invoke-direct {p0}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>()V

    throw p0

    .line 568
    :cond_3
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    invoke-direct {p0}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>()V

    throw p0

    :cond_4
    return-void
.end method

.method public static embedTypeInfo(Lcom/itextpdf/barcodes/qrcode/ErrorCorrectionLevel;ILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 5
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    .line 296
    new-instance v0, Lcom/itextpdf/barcodes/qrcode/BitVector;

    invoke-direct {v0}, Lcom/itextpdf/barcodes/qrcode/BitVector;-><init>()V

    .line 297
    invoke-static {p0, p1, v0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->makeTypeInfoBits(Lcom/itextpdf/barcodes/qrcode/ErrorCorrectionLevel;ILcom/itextpdf/barcodes/qrcode/BitVector;)V

    const/4 p0, 0x0

    move p1, p0

    .line 299
    :goto_0
    invoke-virtual {v0}, Lcom/itextpdf/barcodes/qrcode/BitVector;->size()I

    move-result v1

    if-ge p1, v1, :cond_1

    .line 302
    invoke-virtual {v0}, Lcom/itextpdf/barcodes/qrcode/BitVector;->size()I

    move-result v1

    const/4 v2, 0x1

    sub-int/2addr v1, v2

    sub-int/2addr v1, p1

    invoke-virtual {v0, v1}, Lcom/itextpdf/barcodes/qrcode/BitVector;->at(I)I

    move-result v1

    .line 305
    sget-object v3, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->TYPE_INFO_COORDINATES:[[I

    aget-object v3, v3, p1

    aget v4, v3, p0

    .line 306
    aget v3, v3, v2

    .line 307
    invoke-virtual {p2, v4, v3, v1}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    const/16 v3, 0x8

    if-ge p1, v3, :cond_0

    .line 311
    invoke-virtual {p2}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getWidth()I

    move-result v4

    sub-int/2addr v4, p1

    sub-int/2addr v4, v2

    .line 313
    invoke-virtual {p2, v4, v3, v1}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    goto :goto_1

    .line 317
    :cond_0
    invoke-virtual {p2}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getHeight()I

    move-result v2

    add-int/lit8 v2, v2, -0x7

    add-int/lit8 v4, p1, -0x8

    add-int/2addr v2, v4

    .line 318
    invoke-virtual {p2, v3, v2, v1}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    :goto_1
    add-int/lit8 p1, p1, 0x1

    goto :goto_0

    :cond_1
    return-void
.end method

.method private static embedVerticalSeparationPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 5
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    .line 609
    sget-object v0, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->VERTICAL_SEPARATION_PATTERN:[[I

    const/4 v1, 0x0

    aget-object v2, v0, v1

    array-length v2, v2

    const/4 v3, 0x1

    if-ne v2, v3, :cond_2

    array-length v0, v0

    const/4 v2, 0x7

    if-ne v0, v2, :cond_2

    move v0, v1

    :goto_0
    if-ge v0, v2, :cond_1

    add-int v3, p1, v0

    .line 613
    invoke-virtual {p2, p0, v3}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->get(II)B

    move-result v4

    invoke-static {v4}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->isEmpty(B)Z

    move-result v4

    if-eqz v4, :cond_0

    .line 616
    sget-object v4, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->VERTICAL_SEPARATION_PATTERN:[[I

    aget-object v4, v4, v0

    aget v4, v4, v1

    invoke-virtual {p2, p0, v3, v4}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    add-int/lit8 v0, v0, 0x1

    goto :goto_0

    .line 614
    :cond_0
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    invoke-direct {p0}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>()V

    throw p0

    :cond_1
    return-void

    .line 610
    :cond_2
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    const-string p1, "Bad vertical separation pattern"

    invoke-direct {p0, p1}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>(Ljava/lang/String;)V

    throw p0
.end method

.method public static findMSBSet(I)I
    .locals 1

    const/4 v0, 0x0

    :goto_0
    if-eqz p0, :cond_0

    ushr-int/lit8 p0, p0, 0x1

    add-int/lit8 v0, v0, 0x1

    goto :goto_0

    :cond_0
    return v0
.end method

.method private static isEmpty(B)Z
    .locals 1

    const/4 v0, -0x1

    if-ne p0, v0, :cond_0

    const/4 p0, 0x1

    goto :goto_0

    :cond_0
    const/4 p0, 0x0

    :goto_0
    return p0
.end method

.method private static isValidValue(B)Z
    .locals 2

    const/4 v0, -0x1

    const/4 v1, 0x1

    if-eq p0, v0, :cond_1

    if-eqz p0, :cond_1

    if-ne p0, v1, :cond_0

    goto :goto_0

    :cond_0
    const/4 v1, 0x0

    :cond_1
    :goto_0
    return v1
.end method

.method public static makeTypeInfoBits(Lcom/itextpdf/barcodes/qrcode/ErrorCorrectionLevel;ILcom/itextpdf/barcodes/qrcode/BitVector;)V
    .locals 1
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    .line 501
    invoke-static {p1}, Lcom/itextpdf/barcodes/qrcode/QRCode;->isValidMaskPattern(I)Z

    move-result v0

    if-eqz v0, :cond_1

    .line 504
    invoke-virtual {p0}, Lcom/itextpdf/barcodes/qrcode/ErrorCorrectionLevel;->getBits()I

    move-result p0

    shl-int/lit8 p0, p0, 0x3

    or-int/2addr p0, p1

    const/4 p1, 0x5

    .line 505
    invoke-virtual {p2, p0, p1}, Lcom/itextpdf/barcodes/qrcode/BitVector;->appendBits(II)V

    const/16 p1, 0x537

    .line 507
    invoke-static {p0, p1}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->calculateBCHCode(II)I

    move-result p0

    const/16 p1, 0xa

    .line 508
    invoke-virtual {p2, p0, p1}, Lcom/itextpdf/barcodes/qrcode/BitVector;->appendBits(II)V

    .line 510
    new-instance p0, Lcom/itextpdf/barcodes/qrcode/BitVector;

    invoke-direct {p0}, Lcom/itextpdf/barcodes/qrcode/BitVector;-><init>()V

    const/16 p1, 0x5412

    const/16 v0, 0xf

    .line 511
    invoke-virtual {p0, p1, v0}, Lcom/itextpdf/barcodes/qrcode/BitVector;->appendBits(II)V

    .line 512
    invoke-virtual {p2, p0}, Lcom/itextpdf/barcodes/qrcode/BitVector;->xor(Lcom/itextpdf/barcodes/qrcode/BitVector;)V

    .line 515
    invoke-virtual {p2}, Lcom/itextpdf/barcodes/qrcode/BitVector;->size()I

    move-result p0

    if-ne p0, v0, :cond_0

    return-void

    .line 516
    :cond_0
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    new-instance p1, Ljava/lang/StringBuilder;

    const-string v0, "should not happen but we got: "

    invoke-direct {p1, v0}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p2}, Lcom/itextpdf/barcodes/qrcode/BitVector;->size()I

    move-result p2

    invoke-virtual {p1, p2}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object p1

    invoke-virtual {p1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    invoke-direct {p0, p1}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>(Ljava/lang/String;)V

    throw p0

    .line 502
    :cond_1
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    const-string p1, "Invalid mask pattern"

    invoke-direct {p0, p1}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>(Ljava/lang/String;)V

    throw p0
.end method

.method public static makeVersionInfoBits(ILcom/itextpdf/barcodes/qrcode/BitVector;)V
    .locals 2
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    const/4 v0, 0x6

    .line 531
    invoke-virtual {p1, p0, v0}, Lcom/itextpdf/barcodes/qrcode/BitVector;->appendBits(II)V

    const/16 v0, 0x1f25

    .line 532
    invoke-static {p0, v0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->calculateBCHCode(II)I

    move-result p0

    const/16 v0, 0xc

    .line 533
    invoke-virtual {p1, p0, v0}, Lcom/itextpdf/barcodes/qrcode/BitVector;->appendBits(II)V

    .line 536
    invoke-virtual {p1}, Lcom/itextpdf/barcodes/qrcode/BitVector;->size()I

    move-result p0

    const/16 v0, 0x12

    if-ne p0, v0, :cond_0

    return-void

    .line 537
    :cond_0
    new-instance p0, Lcom/itextpdf/barcodes/exceptions/WriterException;

    new-instance v0, Ljava/lang/StringBuilder;

    const-string v1, "should not happen but we got: "

    invoke-direct {v0, v1}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p1}, Lcom/itextpdf/barcodes/qrcode/BitVector;->size()I

    move-result p1

    invoke-virtual {v0, p1}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object p1

    invoke-virtual {p1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    invoke-direct {p0, p1}, Lcom/itextpdf/barcodes/exceptions/WriterException;-><init>(Ljava/lang/String;)V

    throw p0
.end method

.method private static maybeEmbedPositionAdjustmentPatterns(ILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 7
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    const/4 v0, 0x2

    if-ge p0, v0, :cond_0

    return-void

    :cond_0
    add-int/lit8 p0, p0, -0x1

    .line 696
    sget-object v0, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->POSITION_ADJUSTMENT_PATTERN_COORDINATE_TABLE:[[I

    aget-object p0, v0, p0

    .line 697
    array-length v0, p0

    const/4 v1, 0x0

    move v2, v1

    :goto_0
    if-ge v2, v0, :cond_4

    move v3, v1

    :goto_1
    if-ge v3, v0, :cond_3

    .line 700
    aget v4, p0, v2

    .line 701
    aget v5, p0, v3

    const/4 v6, -0x1

    if-eq v5, v6, :cond_2

    if-ne v4, v6, :cond_1

    goto :goto_2

    .line 706
    :cond_1
    invoke-virtual {p1, v5, v4}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->get(II)B

    move-result v6

    invoke-static {v6}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->isEmpty(B)Z

    move-result v6

    if-eqz v6, :cond_2

    add-int/lit8 v5, v5, -0x2

    add-int/lit8 v4, v4, -0x2

    .line 709
    invoke-static {v5, v4, p1}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->embedPositionAdjustmentPattern(IILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V

    :cond_2
    :goto_2
    add-int/lit8 v3, v3, 0x1

    goto :goto_1

    :cond_3
    add-int/lit8 v2, v2, 0x1

    goto :goto_0

    :cond_4
    return-void
.end method

.method public static maybeEmbedVersionInfo(ILcom/itextpdf/barcodes/qrcode/ByteMatrix;)V
    .locals 6
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lcom/itextpdf/barcodes/exceptions/WriterException;
        }
    .end annotation

    const/4 v0, 0x7

    if-ge p0, v0, :cond_0

    return-void

    .line 343
    :cond_0
    new-instance v0, Lcom/itextpdf/barcodes/qrcode/BitVector;

    invoke-direct {v0}, Lcom/itextpdf/barcodes/qrcode/BitVector;-><init>()V

    .line 344
    invoke-static {p0, v0}, Lcom/itextpdf/barcodes/qrcode/MatrixUtil;->makeVersionInfoBits(ILcom/itextpdf/barcodes/qrcode/BitVector;)V

    const/4 p0, 0x0

    const/16 v1, 0x11

    move v2, p0

    :goto_0
    const/4 v3, 0x6

    if-ge v2, v3, :cond_2

    move v3, p0

    :goto_1
    const/4 v4, 0x3

    if-ge v3, v4, :cond_1

    .line 352
    invoke-virtual {v0, v1}, Lcom/itextpdf/barcodes/qrcode/BitVector;->at(I)I

    move-result v4

    add-int/lit8 v1, v1, -0x1

    .line 356
    invoke-virtual {p1}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getHeight()I

    move-result v5

    add-int/lit8 v5, v5, -0xb

    add-int/2addr v5, v3

    invoke-virtual {p1, v2, v5, v4}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    .line 359
    invoke-virtual {p1}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->getHeight()I

    move-result v5

    add-int/lit8 v5, v5, -0xb

    add-int/2addr v5, v3

    invoke-virtual {p1, v5, v2, v4}, Lcom/itextpdf/barcodes/qrcode/ByteMatrix;->set(III)V

    add-int/lit8 v3, v3, 0x1

    goto :goto_1

    :cond_1
    add-int/lit8 v2, v2, 0x1

    goto :goto_0

    :cond_2
    return-void
.end method
