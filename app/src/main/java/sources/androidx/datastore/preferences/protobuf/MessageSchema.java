...
final class MessageSchema<T> implements Schema<T> {
    ...
    static <T> MessageSchema<T> newSchemaForMessageInfo(StructuralMessageInfo messageInfo, NewInstanceSchema newInstanceSchema, ListFieldSchema listFieldSchema, UnknownFieldSchema<?, ?> unknownFieldSchema, ExtensionSchema<?> extensionSchema, MapFieldSchema mapFieldSchema) {
        ...
        for (FieldInfo fieldInfo : fields) {
            if (fieldInfo.getType() == FieldType.MAP) {
                i2++;
            } else if (fieldInfo.getType().m50id() >= 18 && fieldInfo.getType().m50id() <= 49) {
                i3++;
            }
        }
        ...
        while (i4 < fields.length) {
            ...
            if (fieldInfo2.getType() == FieldType.MAP) {
                ...
                if (fieldInfo2.getType().m50id() >= 18 && fieldInfo2.getType().m50id() <= 49) {
                    i = i5;
    ...
    */
    ...
    private int getSerializedSizeProto2(T message) {
        ...
        while (i5 < this.buffer.length) {
            ...
            if (iType <= 17) {
                ...
                i = (!this.useCachedSizeField || iType < FieldType.DOUBLE_LIST_PACKED.m50id() || iType > FieldType.SINT64_LIST_PACKED.m50id()) ? 0 : this.buffer[i5 + 2] & i3;
                i2 = 0;
            }
            long jOffset = offset(iTypeAndOffsetAt);
            switch (iType) {
    ...
    private int getSerializedSizeProto3(T message) {
        ...
        for (int i2 = 0; i2 < this.buffer.length; i2 += 3) {
            ...
            long jOffset = offset(iTypeAndOffsetAt);
            int i3 = (iType < FieldType.DOUBLE_LIST_PACKED.m50id() || iType > FieldType.SINT64_LIST_PACKED.m50id()) ? 0 : this.buffer[i2 + 2] & 1048575;
            switch (iType) {
                case 0:
    ...
    */
     ...
     */
    /* JADX WARN: Multi-variable type inference failed */
    /* JADX WARN: Removed duplicated region for block: B:169:0x0646 A[Catch: all -> 0x069e, TRY_LEAVE, TryCatch #13 {all -> 0x069e, blocks: (B:156:0x0617, B:167:0x0640, B:169:0x0646, B:179:0x066e, B:180:0x0673), top: B:208:0x0617 }] */
    ...
    */
    ...
    private int decodeMapEntryValue(byte[] data, int position, int limit, WireFormat.FieldType fieldType, Class<?> messageType, ArrayDecoders.Registers registers) throws IOException {
        switch (C04781.$SwitchMap$com$google$protobuf$WireFormat$FieldType[fieldType.ordinal()]) {
            case 1:
