...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    void setState(TransitionState newState) {
        ...
        int i = C02415.f60xabc7e4ac[transitionState.ordinal()];
        if (i == 1 || i == 2) {
    ...
    static /* synthetic */ class C02415 {
        ...
        static {
            ...
            try {
                f60xabc7e4ac[TransitionState.SETUP.ordinal()] = 2;
            } catch (NoSuchFieldError unused2) {
            }
            try {
                f60xabc7e4ac[TransitionState.MOVING.ordinal()] = 3;
            } catch (NoSuchFieldError unused3) {
            }
            try {
                f60xabc7e4ac[TransitionState.FINISHED.ordinal()] = 4;
    ...
    private static class MyTracker implements MotionTracker {
        ...
        private static MyTracker f61me = new MyTracker();
        ...
        public static MyTracker obtain() {
            f61me.tracker = VelocityTracker.obtain();
            return f61me;
        }
    ...
    class Model {
        ...
        private void debugWidget(String str, ConstraintWidget child) {
            ...
            if (child.mTop.mTarget != null) {
                strConcat = "T".concat(child.mTop.mTarget.mType == ConstraintAnchor.Type.TOP ? "T" : SvgConstants.Attributes.PATH_DATA_BEARING);
            } else {
                strConcat = "__";
            }
            StringBuilder sbAppend = new StringBuilder().append(sb.append(strConcat).toString());
            if (child.mBottom.mTarget != null) {
                strConcat2 = SvgConstants.Attributes.PATH_DATA_BEARING.concat(child.mBottom.mTarget.mType != ConstraintAnchor.Type.TOP ? SvgConstants.Attributes.PATH_DATA_BEARING : "T");
            } else {
                strConcat2 = "__";
            }
            StringBuilder sbAppend2 = new StringBuilder().append(sbAppend.append(strConcat2).toString());
            if (child.mLeft.mTarget != null) {
                strConcat3 = "L".concat(child.mLeft.mTarget.mType == ConstraintAnchor.Type.LEFT ? "L" : SvgConstants.Attributes.PATH_DATA_CATMULL_CURVE);
            } else {
                strConcat3 = "__";
            }
            StringBuilder sbAppend3 = new StringBuilder().append(sbAppend2.append(strConcat3).toString());
            if (child.mRight.mTarget != null) {
                strConcat4 = SvgConstants.Attributes.PATH_DATA_CATMULL_CURVE.concat(child.mRight.mTarget.mType != ConstraintAnchor.Type.LEFT ? SvgConstants.Attributes.PATH_DATA_CATMULL_CURVE : "L");
            }
        ...
        private void debugLayout(String title, ConstraintWidgetContainer c) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                String str3 = (((constraintWidget.mTop.mTarget != null ? "T" : "_") + (constraintWidget.mBottom.mTarget != null ? SvgConstants.Attributes.PATH_DATA_BEARING : "_")) + (constraintWidget.mLeft.mTarget != null ? "L" : "_")) + (constraintWidget.mRight.mTarget != null ? SvgConstants.Attributes.PATH_DATA_CATMULL_CURVE : "_");
    ...
    */
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray typedArrayObtainStyledAttributes = getContext().obtainStyledAttributes(attrs, C0257R.styleable.MotionLayout);
            int indexCount = typedArrayObtainStyledAttributes.getIndexCount();
            boolean z = true;
            for (int i = 0; i < indexCount; i++) {
                int index = typedArrayObtainStyledAttributes.getIndex(i);
                if (index == C0257R.styleable.MotionLayout_layoutDescription) {
                    this.mScene = new MotionScene(getContext(), this, typedArrayObtainStyledAttributes.getResourceId(index, -1));
                } else if (index == C0257R.styleable.MotionLayout_currentState) {
                    this.mCurrentState = typedArrayObtainStyledAttributes.getResourceId(index, -1);
                } else if (index == C0257R.styleable.MotionLayout_motionProgress) {
                    this.mTransitionGoalPosition = typedArrayObtainStyledAttributes.getFloat(index, 0.0f);
                    this.mInTransition = true;
                } else if (index == C0257R.styleable.MotionLayout_applyMotionScene) {
                    z = typedArrayObtainStyledAttributes.getBoolean(index, z);
                } else if (index == C0257R.styleable.MotionLayout_showPaths) {
                    if (this.mDebugPath == 0) {
                        this.mDebugPath = typedArrayObtainStyledAttributes.getBoolean(index, false) ? 2 : 0;
                    }
                } else if (index == C0257R.styleable.MotionLayout_motionDebug) {
    ...
    public void updateStateAnimate(int stateId, ConstraintSet set, int duration) {
        if (this.mScene != null && this.mCurrentState == stateId) {
            updateState(C0257R.id.view_transition, getConstraintSet(stateId));
            setState(C0257R.id.view_transition, -1, -1);
            updateState(stateId, set);
            MotionScene.Transition transition = new MotionScene.Transition(-1, this.mScene, C0257R.id.view_transition, stateId);
