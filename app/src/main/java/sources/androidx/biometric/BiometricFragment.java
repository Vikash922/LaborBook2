...
public class BiometricFragment extends Fragment {
    ...
    void authenticate(BiometricPrompt.PromptInfo promptInfo, BiometricPrompt.CryptoObject cryptoObject) {
        ...
        if (isManagingDeviceCredentialButton()) {
            this.mViewModel.setNegativeButtonTextOverride(getString(C0178R.string.confirm_device_credential_password));
        } else {
    ...
    void authenticateWithBiometricPrompt(android.hardware.biometrics.BiometricPrompt biometricPrompt, Context context) {
        ...
        try {
            ...
            if (context != null) {
                string = context.getString(C0178R.string.default_error_msg);
            } else {
                string = "";
            }
    ...
    void onAuthenticationError(final int i, final CharSequence charSequence) {
        ...
        if (!isUsingFingerprintDialog()) {
            if (charSequence == null) {
                charSequence = getString(C0178R.string.default_error_msg) + StringUtils.SPACE + i;
            }
    ...
    void onAuthenticationFailed() {
        if (isUsingFingerprintDialog()) {
            showFingerprintErrorMessage(getString(C0178R.string.fingerprint_not_recognized));
        }
        sendFailureToClient();
    }
    ...
    void onCancelButtonPressed() {
        ...
        if (negativeButtonText == null) {
            negativeButtonText = getString(C0178R.string.default_error_msg);
        }
    ...
    private void launchConfirmCredentialActivity() {
        ...
        if (keyguardManager == null) {
            sendErrorAndDismiss(12, getString(C0178R.string.generic_error_no_keyguard));
            return;
        }
        CharSequence title = this.mViewModel.getTitle();
        CharSequence subtitle = this.mViewModel.getSubtitle();
        ...
        if (intentCreateConfirmDeviceCredentialIntent == null) {
            sendErrorAndDismiss(14, getString(C0178R.string.generic_error_no_device_credential));
            return;
        }
    ...
    private void handleConfirmCredentialResult(int i) {
        if (i == -1) {
            ...
            sendErrorAndDismiss(10, getString(C0178R.string.generic_error_user_canceled));
        }
    }

    private void showFingerprintErrorMessage(CharSequence charSequence) {
        if (charSequence == null) {
            charSequence = getString(C0178R.string.default_error_msg);
        }
