...
public class GoogleApiAvailability extends GoogleApiAvailabilityLight {
    ...
    final Dialog zaa(Context context, int i, zag zagVar, DialogInterface.OnCancelListener onCancelListener, DialogInterface.OnClickListener onClickListener) {
        ...
        if (strZac != null) {
            ...
            builder.setPositiveButton(strZac, onClickListener2);
        }
    ...
    final void zae(Context context, int i, String str, PendingIntent pendingIntent) {
        ...
        if (DeviceProperties.isWearable(context)) {
            ...
            if (DeviceProperties.isWearableWithoutPlayStore(context)) {
                style.addAction(C1319R.drawable.common_full_open_on_phone, resources.getString(C1319R.string.common_open_on_phone), pendingIntent);
            } else {
                style.setContentIntent(pendingIntent);
            }
        } else {
            style.setSmallIcon(android.R.drawable.stat_sys_warning).setTicker(resources.getString(C1319R.string.common_google_play_services_notification_ticker)).setWhen(System.currentTimeMillis()).setContentIntent(pendingIntent).setContentText(strZae);
        }
        if (PlatformVersion.isAtLeastO()) {
    ...
    public final boolean zag(Activity activity, LifecycleFragment lifecycleFragment, int i, int i2, DialogInterface.OnCancelListener onCancelListener) {
        Dialog dialogZaa = zaa(activity, i, zag.zad(lifecycleFragment, getErrorResolutionIntent(activity, i, SvgConstants.Attributes.f3323D), 2), onCancelListener, null);
        if (dialogZaa == null) {
            return false;
        }
    ...
    public Dialog getErrorDialog(Activity activity, int i, int i2, DialogInterface.OnCancelListener onCancelListener) {
        return zaa(activity, i, zag.zab(activity, getErrorResolutionIntent(activity, i, SvgConstants.Attributes.f3323D), i2), onCancelListener, null);
    }
    ...
    public Dialog getErrorDialog(Fragment fragment, int i, int i2, DialogInterface.OnCancelListener onCancelListener) {
        return zaa(fragment.requireContext(), i, zag.zac(fragment, getErrorResolutionIntent(fragment.requireContext(), i, SvgConstants.Attributes.f3323D), i2), onCancelListener, null);
    }
}
