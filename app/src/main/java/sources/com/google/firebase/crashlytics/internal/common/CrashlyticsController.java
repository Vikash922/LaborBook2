...
class CrashlyticsController {
    ...
    synchronized void handleUncaughtException(final SettingsProvider settingsProvider, final Thread thread, final Throwable th, final boolean z) {
        Logger.getLogger().m462d("Handling uncaught exception \"" + th + "\" from thread " + thread.getName());
        final long jCurrentTimeMillis = System.currentTimeMillis();
        Task taskSubmitTask = this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2
            ...
            public Task<Void> call() throws Exception {
                    ...
                    if (CrashlyticsController.this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
                        ...
                        return settingsProvider.getSettingsAsync().onSuccessTask(executor, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2.1
                            ...
                            public Task<Void> then(Settings settings) throws Exception {
                                if (settings == null) {
                                    Logger.getLogger().m470w("Received null app settings, cannot send reports at crash time.");
                ...
                Logger.getLogger().m464e("Tried to write a fatal exception while no session was open.");
                return Tasks.forResult(null);
            }
        });
        if (!z) {
            try {
                ...
                Logger.getLogger().m464e("Cannot send reports. Timed out while fetching settings.");
            } catch (Exception e) {
                Logger.getLogger().m465e("Error handling uncaught exception", e);
            }
        }
    }
    ...
    private Task<Boolean> waitForReportAction() {
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            Logger.getLogger().m462d("Automatic data collection is enabled. Allowing upload.");
            this.unsentReportsAvailable.trySetResult(false);
            return Tasks.forResult(true);
        }
        Logger.getLogger().m462d("Automatic data collection is disabled.");
        Logger.getLogger().m468v("Notifying that unsent reports are available.");
        ...
        Logger.getLogger().m462d("Waiting for send/deleteUnsentReports to be called.");
    ...
    boolean didCrashOnPreviousExecution() {
        ...
        Logger.getLogger().m468v("Found previous crash marker.");
    ...
    Task<Boolean> checkForUnsentReports() {
        if (!this.checkForUnsentReportsCalled.compareAndSet(false, true)) {
            Logger.getLogger().m470w("checkForUnsentReports should only be called once per execution.");
    ...
    Task<Void> submitAllReports(Task<Settings> task) {
        if (!this.reportingCoordinator.hasReportsToSend()) {
            Logger.getLogger().m468v("No crash reports are available to be sent.");
            this.unsentReportsAvailable.trySetResult(false);
            return Tasks.forResult(null);
        }
        Logger.getLogger().m468v("Crash reports are available to be sent.");
        return waitForReportAction().onSuccessTask(new C22264(task));
    }

    /* JADX INFO: renamed from: com.google.firebase.crashlytics.internal.common.CrashlyticsController$4 */
    /* JADX INFO: loaded from: classes6.dex */
    class C22264 implements SuccessContinuation<Boolean, Void> {
            ...
            return CrashlyticsController.this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1
                ...
                public Task<Void> call() throws Exception {
                    if (!bool.booleanValue()) {
                        Logger.getLogger().m468v("Deleting cached crash reports...");
                        CrashlyticsController.deleteFiles(CrashlyticsController.this.listAppExceptionMarkerFiles());
                        CrashlyticsController.this.reportingCoordinator.removeAllReports();
                        CrashlyticsController.this.unsentReportsHandled.trySetResult(null);
                        return Tasks.forResult(null);
                    }
                    Logger.getLogger().m462d("Sending cached crash reports...");
                    CrashlyticsController.this.dataCollectionArbiter.grantDataCollectionPermission(bool.booleanValue());
                    final Executor executor = CrashlyticsController.this.backgroundWorker.getExecutor();
                    return C22264.this.val$settingsDataTask.onSuccessTask(executor, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1.1
                        @Override // com.google.android.gms.tasks.SuccessContinuation
                        public Task<Void> then(Settings settings) throws Exception {
                            if (settings != null) {
                            ...
                            Logger.getLogger().m470w("Received null app settings at app startup. Cannot send cached reports");
                            return Tasks.forResult(null);
                        }
                    });
                }
        ...
        this.backgroundWorker.submit(new Runnable() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.6
            ...
            public void run() {
                ...
                if (currentSessionId != null) {
                    ...
                    Logger.getLogger().m470w("Tried to write a non-fatal exception while no session was open.");
                }
            }
        });
    }
    ...
    void logFatalException(Thread thread, Throwable th) {
        ...
        if (settingsProvider == null) {
            Logger.getLogger().m470w("settingsProvider not set");
        } else {
    ...
    void setCustomKey(String str, String str2) {
        try {
            ...
            Logger.getLogger().m464e("Attempting to set custom attribute with null key, ignoring.");
        }
    }
    ...
    boolean finalizeSessions(SettingsProvider settingsProvider) {
        ...
        if (isHandlingException()) {
            Logger.getLogger().m470w("Skipping session finalization because a crash has already occurred.");
            return Boolean.FALSE.booleanValue();
        }
        Logger.getLogger().m468v("Finalizing previously open sessions.");
        try {
            doCloseSessions(true, settingsProvider);
            Logger.getLogger().m468v("Closed all previously open sessions.");
            return true;
        } catch (Exception e) {
            Logger.getLogger().m465e("Unable to finalize previously open sessions.", e);
            return false;
        }
    }
    ...
    public void doOpenSession(String str, Boolean bool) {
        ...
        Logger.getLogger().m462d("Opening a new session with ID " + str);
    ...
    private void doCloseSessions(boolean z, SettingsProvider settingsProvider) {
        ...
        if (arrayList.size() <= z) {
            Logger.getLogger().m468v("No open sessions to be closed.");
            return;
        }
        ...
        if (settingsProvider.getSettingsSync().featureFlagData.collectAnrs) {
            ...
            Logger.getLogger().m468v("ANR feature disabled.");
        }
    ...
    void saveVersionControlInfo() {
        try {
            ...
            if (versionControlInfo != null) {
                ...
                Logger.getLogger().m466i("Saved version control info");
            }
        } catch (IOException e) {
            Logger.getLogger().m471w("Unable to save version control info", e);
        }
    }
    ...
    String getVersionControlInfo() throws IOException {
        ...
        Logger.getLogger().m462d("Read version control info");
    ...
    private InputStream getResourceAsStream(String str) {
        ...
        if (classLoader == null) {
            Logger.getLogger().m470w("Couldn't get Class Loader");
            return null;
        }
        ...
        Logger.getLogger().m466i("No version control information found");
        return null;
    }
    ...
    private void finalizePreviousNativeSession(String str) {
        Logger.getLogger().m468v("Finalizing native report for session " + str);
        ...
        if (nativeCoreAbsent(str, minidumpFile, applicationExitInto)) {
            Logger.getLogger().m470w("No native core present");
            return;
        }
        ...
        if (!nativeSessionDir.isDirectory()) {
            Logger.getLogger().m470w("Couldn't create directory to store native session files, aborting.");
            return;
        }
        ...
        Logger.getLogger().m462d("CrashlyticsController#finalizePreviousNativeSession");
    ...
    private static boolean nativeCoreAbsent(String str, File file, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        if (file == null || !file.exists()) {
            Logger.getLogger().m470w("No minidump data found for session " + str);
        }
        if (applicationExitInfo == null) {
            Logger.getLogger().m466i("No Tombstones data found for session " + str);
        }
    ...
    public void doWriteAppExceptionMarker(long j) {
        try {
            ...
            Logger.getLogger().m471w("Could not create app exception marker file.", e);
        }
    }
    ...
    public Task<Void> logAnalyticsAppExceptionEvents() {
        ...
        for (File file : listAppExceptionMarkerFiles()) {
            try {
                ...
                Logger.getLogger().m470w("Could not parse app exception timestamp from file " + file.getName());
            }
            file.delete();
        }
    ...
    private Task<Void> logAnalyticsAppExceptionEvent(final long j) {
        if (firebaseCrashExists()) {
            Logger.getLogger().m470w("Skipping logging Crashlytics event to Firebase, FirebaseCrash exists");
            return Tasks.forResult(null);
        }
        Logger.getLogger().m462d("Logging app exception event to Firebase Analytics");
    ...
    static List<NativeSessionFile> getNativeSessionFiles(NativeSessionFileProvider nativeSessionFileProvider, String str, FileStore fileStore, byte[] bArr) {
        ...
        arrayList.add(new FileBackedNativeSessionFile("crash_meta_file", SvgConstants.Tags.METADATA, nativeSessionFileProvider.getMetadataFile()));
    ...
    private void writeApplicationExitInfoEventIfRelevant(String str) {
        if (Build.VERSION.SDK_INT >= 30) {
            ...
            Logger.getLogger().m468v("No ApplicationExitInfo available. Session: " + str);
            return;
        }
        Logger.getLogger().m468v("ANR feature enabled, but device is API " + Build.VERSION.SDK_INT);
    }
}
