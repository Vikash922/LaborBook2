...
public class SettingsController implements SettingsProvider {
                ...
                if (cachedSettings != null) {
                    ...
                    if (settingsJson != null) {
                        ...
                        if (SettingsCacheBehavior.IGNORE_CACHE_EXPIRATION.equals(settingsCacheBehavior) || !settingsJson.isExpired(currentTimeMillis)) {
                            try {
                                Logger.getLogger().m468v("Returning cached settings.");
                                settings = settingsJson;
                            } catch (Exception e) {
                                e = e;
                                settings = settingsJson;
                                Logger.getLogger().m465e("Failed to get cached settings", e);
                            }
                        } else {
                            Logger.getLogger().m468v("Cached settings have expired.");
                        }
                    } else {
                        Logger.getLogger().m465e("Failed to parse cached settings data.", null);
                    }
                } else {
                    Logger.getLogger().m462d("No cached settings data found.");
                }
            }
        } catch (Exception e2) {
            e = e2;
    ...
    public void logSettings(JSONObject jSONObject, String str) throws JSONException {
        Logger.getLogger().m462d(str + jSONObject.toString());
    }
