...
public class FirebaseCrashlytics {
    ...
    static FirebaseCrashlytics init(FirebaseApp firebaseApp, FirebaseInstallationsApi firebaseInstallationsApi, Deferred<CrashlyticsNativeComponent> deferred, Deferred<AnalyticsConnector> deferred2, Deferred<FirebaseRemoteConfigInterop> deferred3, ExecutorService executorService, ExecutorService executorService2) {
        ...
        Logger.getLogger().m466i("Initializing Firebase Crashlytics " + CrashlyticsCore.getVersion() + " for " + packageName);
        ...
        Logger.getLogger().m462d("Mapping file ID is: " + mappingFileId);
        for (BuildIdInfo buildIdInfo2 : buildIdInfo) {
            Logger.getLogger().m462d(String.format("Build id for %s on %s: %s", buildIdInfo2.getLibraryName(), buildIdInfo2.getArch(), buildIdInfo2.getBuildId()));
        }
        try {
            ...
            Logger.getLogger().m468v("Installer package name is: " + appDataCreate.installerPackageName);
            ...
            settingsControllerCreate.loadSettingsData(executorBuildSequentialExecutor).continueWith(executorBuildSequentialExecutor, new Continuation<Void, Object>() { // from class: com.google.firebase.crashlytics.FirebaseCrashlytics.1
                @Override // com.google.android.gms.tasks.Continuation
                public Object then(Task<Void> task) throws Exception {
                    if (task.isSuccessful()) {
                        return null;
                    }
                    Logger.getLogger().m465e("Error fetching settings.", task.getException());
                    return null;
                }
            });
            ...
            Logger.getLogger().m465e("Error retrieving app package info.", e);
            return null;
        }
    }
    ...
    public void recordException(Throwable th) {
        if (th == null) {
            Logger.getLogger().m470w("A null value was passed to recordException. Ignoring.");
        } else {
