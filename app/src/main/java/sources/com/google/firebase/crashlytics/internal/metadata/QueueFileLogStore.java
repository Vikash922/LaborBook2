...
class QueueFileLogStore implements FileLogStore {
    ...
    private LogBytes getLogBytes() {
        ...
        try {
            ...
            Logger.getLogger().m465e("A problem occurred while reading the Crashlytics log file.", e);
        }
    ...
    private void openLogFile() {
        if (this.logFile == null) {
            try {
                ...
                Logger.getLogger().m465e("Could not open log file: " + this.workingFile, e);
            }
        }
    }
    ...
    private void doWriteToLog(long j, String str) {
        ...
        try {
            ...
            this.logFile.add(String.format(Locale.US, "%d %s%n", Long.valueOf(j), str.replaceAll(StringUtils.f4767CR, StringUtils.SPACE).replaceAll(StringUtils.f4768LF, StringUtils.SPACE)).getBytes(UTF_8));
            while (!this.logFile.isEmpty() && this.logFile.usedBytes() > this.maxLogSize) {
                this.logFile.remove();
            }
        } catch (IOException e) {
            Logger.getLogger().m465e("There was a problem writing to the Crashlytics log.", e);
        }
    }
}
