        ...
        while (i4 < length) {
            ...
            if (iIndexOf < 0) {
                ...
                if (i != 0) {
                    ...
                    if (z2) {
                        strReplaceAll = UNESCAPED_SEMICOLONS.matcher(strReplaceAll).replaceAll(StringUtils.f4768LF).trim();
                    }
                } else {
                    if (z2) {
                        strSubstring = UNESCAPED_SEMICOLONS.matcher(strSubstring).replaceAll(StringUtils.f4768LF).trim();
                    }
                    strReplaceAll = VCARD_ESCAPES.matcher(NEWLINE_ESCAPE.matcher(CR_LF_SPACE_TAB.matcher(strSubstring).replaceAll("")).replaceAll(StringUtils.f4768LF)).replaceAll("$1");
                }
