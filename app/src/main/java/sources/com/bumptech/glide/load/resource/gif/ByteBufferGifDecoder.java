...
public class ByteBufferGifDecoder implements ResourceDecoder<ByteBuffer, GifDrawable> {
    ...
    private static int getSampleSize(GifHeader gifHeader, int i, int i2) {
        ...
        if (Log.isLoggable(TAG, 2) && iMax > 1) {
            Log.v(TAG, "Downsampling GIF, sampleSize: " + iMax + ", target dimens: [" + i + SvgConstants.Attributes.f3330X + i2 + "], actual dimens: [" + gifHeader.getWidth() + SvgConstants.Attributes.f3330X + gifHeader.getHeight() + "]");
        }
        return iMax;
    }
