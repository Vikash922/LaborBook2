...
class EngineJob<R> implements DecodeJob.Callback<R>, FactoryPools.Poolable {
    ...
    void notifyCallbacksOfResult() {
        synchronized (this) {
            ...
            for (ResourceCallbackAndExecutor resourceCallbackAndExecutor : resourceCallbacksAndExecutorsCopy) {
                resourceCallbackAndExecutor.executor.execute(new CallResourceReady(resourceCallbackAndExecutor.f196cb));
            }
    ...
    void notifyCallbacksOfException() {
        synchronized (this) {
            ...
            for (ResourceCallbackAndExecutor resourceCallbackAndExecutor : resourceCallbacksAndExecutorsCopy) {
                resourceCallbackAndExecutor.executor.execute(new CallLoadFailed(resourceCallbackAndExecutor.f196cb));
            }
            decrementPendingCallbacks();
        }
    }
    ...
    private class CallLoadFailed implements Runnable {
        ...
        private final ResourceCallback f194cb;

        CallLoadFailed(ResourceCallback resourceCallback) {
            this.f194cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f194cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f194cb)) {
                        EngineJob.this.callCallbackOnLoadFailed(this.f194cb);
                    }
    ...
    private class CallResourceReady implements Runnable {
        ...
        /* JADX INFO: renamed from: cb */
        private final ResourceCallback f195cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f195cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f195cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f195cb)) {
                        EngineJob.this.engineResource.acquire();
                        EngineJob.this.callCallbackOnResourceReady(this.f195cb);
                        EngineJob.this.removeCallback(this.f195cb);
                    }
                    EngineJob.this.decrementPendingCallbacks();
                }
            }
    ...
    static final class ResourceCallbackAndExecutor {
        ...
        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f196cb.equals(((ResourceCallbackAndExecutor) obj).f196cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f196cb.hashCode();
        }
    }
