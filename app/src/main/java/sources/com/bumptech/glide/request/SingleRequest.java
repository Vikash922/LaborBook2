    ...
    */
    ...
    private void onResourceReady(Resource<R> resource, R r, DataSource dataSource, boolean z) {
        ...
        if (this.glideContext.getLogLevel() <= 3) {
            Log.d(GLIDE_TAG, "Finished loading " + r.getClass().getSimpleName() + " from " + dataSource + " for " + this.model + " with size [" + this.width + SvgConstants.Attributes.f3330X + this.height + "] in " + LogTime.getElapsedMillis(this.startTime) + " ms");
        }
    ...
    private void onLoadFailed(GlideException glideException, int i) {
        ...
        synchronized (this.requestLock) {
            ...
            if (logLevel <= i) {
                Log.w(GLIDE_TAG, "Load failed for [" + this.model + "] with dimensions [" + this.width + SvgConstants.Attributes.f3330X + this.height + "]", glideException);
                if (logLevel <= 4) {
