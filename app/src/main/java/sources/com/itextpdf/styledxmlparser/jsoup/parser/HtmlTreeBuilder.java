...
public class HtmlTreeBuilder extends TreeBuilder {
    ...
    List<Node> parseFragment(String str, Element element, String str2, Parser parser) {
        ...
        if (element != null) {
            ...
            if (StringUtil.m2669in(strNormalName, "title", "textarea")) {
                this.tokeniser.transition(TokeniserState.Rcdata);
            } else if (StringUtil.m2669in(strNormalName, "iframe", "noembed", "noframes", "style", "xmp")) {
            ...
            element2 = new Element(Tag.valueOf(strNormalName, this.settings), str2);
    ...
    void error(HtmlTreeBuilderState htmlTreeBuilderState) {
        if (this.parser.getErrors().canAddError()) {
            this.parser.getErrors().add(new ParseError(this.reader.pos(), "Unexpected token [{0}] when in state [{1}]", this.currentToken.tokenType(), htmlTreeBuilderState));
        }
    }
    ...
    Element insert(Token.StartTag startTag) {
        ...
        if (startTag.isSelfClosing()) {
            ...
            this.tokeniser.transition(TokeniserState.Data);
            this.tokeniser.emit(((Token.Tag) this.emptyEnd.reset()).name(elementInsertEmpty.tagName()));
    ...
    private void clearStackToContext(String... strArr) {
        for (int size = this.stack.size() - 1; size >= 0; size--) {
            Element element = this.stack.get(size);
            if (StringUtil.m2669in(element.normalName(), strArr) || element.normalName().equals("html")) {
                return;
            }
            this.stack.remove(size);
        }
    ...
    void resetInsertionMode() {
        ...
        for (int size = this.stack.size() - 1; size >= 0; size--) {
            ...
            String strNormalName = element != null ? element.normalName() : "";
            ...
            if (XfdfConstants.HEAD.equals(strNormalName)) {
                transition(HtmlTreeBuilderState.InBody);
                return;
            }
            if (SDKConstants.PARAM_A2U_BODY.equals(strNormalName)) {
                transition(HtmlTreeBuilderState.InBody);
                return;
            }
    ...
    void insertInFosterParent(Node node) {
        ...
        if (fromStack != null) {
            if (fromStack.parent() != null) {
                elementAboveOnStack = (Element) fromStack.parent();
                z = true;
            } else {
    ...
    protected boolean isContentForTagData(String str) {
        return str.equals(SvgConstants.Tags.SCRIPT) || str.equals("style");
    }
}
