...
public class JsoupXmlParser implements IXmlParser {
    ...
    public IDocumentNode parse(InputStream inputStream, String str) throws IOException {
        INode iNodeWrapJsoupHierarchy = wrapJsoupHierarchy(Jsoup.parse(inputStream, str, "", Parser.xmlParser()));
    ...
    public IDocumentNode parse(String str) {
        INode iNodeWrapJsoupHierarchy = wrapJsoupHierarchy(Jsoup.parse(str, "", Parser.xmlParser()));
        if (iNodeWrapJsoupHierarchy instanceof IDocumentNode) {
            return (IDocumentNode) iNodeWrapJsoupHierarchy;
        }
        throw new IllegalStateException();
