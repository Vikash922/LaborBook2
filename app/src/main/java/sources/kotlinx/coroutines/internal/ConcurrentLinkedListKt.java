...
public final class ConcurrentLinkedListKt {
    ...
    public static final <S extends Segment<S>> Object findSegmentInternal(S s, long j, Function2<? super Long, ? super S, ? extends S> function2) {
        while (true) {
            if (s.id >= j && !s.isRemoved()) {
                return SegmentOrClosed.m4877constructorimpl(s);
            }
            Object nextOrClosed = s.getNextOrClosed();
            if (nextOrClosed == CLOSED) {
                return SegmentOrClosed.m4877constructorimpl(CLOSED);
            }
    ...
    public static final /* synthetic */ <S extends Segment<S>> boolean moveForward$atomicfu(Object obj, AtomicReferenceFieldUpdater atomicReferenceFieldUpdater, S s) {
        while (true) {
            ...
            if (C0214xc40028dd.m8m(atomicReferenceFieldUpdater, obj, segment, s)) {
    ...
    public static final /* synthetic */ <S extends Segment<S>> boolean moveForward$atomicfu$array(AtomicReferenceArray atomicReferenceArray, int i, S s) {
        while (true) {
            ...
            if (C2185xa7a47114.m442m(atomicReferenceArray, i, segment, s)) {
                if (segment.decPointers$kotlinx_coroutines_core()) {
                    segment.remove();
                }
                return true;
        ...
        loop0: while (true) {
            ...
            if (!SegmentOrClosed.m4882isClosedimpl(objFindSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (C0214xc40028dd.m8m(atomicReferenceFieldUpdater, obj, segment, segmentM4880getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
        ...
        loop0: while (true) {
            ...
            if (!SegmentOrClosed.m4882isClosedimpl(objFindSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (C2185xa7a47114.m442m(atomicReferenceArray, i, segment, segmentM4880getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (segmentM4880getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
