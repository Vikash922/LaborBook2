...
public final class FlowableScalarXMap {
    ...
    public static <T, R> boolean tryScalarXMapSubscribe(Publisher<T> publisher, Subscriber<? super R> subscriber, Function<? super T, ? extends Publisher<? extends R>> function) {
        ...
        try {
            Manifest.permission_group permission_groupVar = (Object) ((Supplier) publisher).get();
            if (permission_groupVar == null) {
                EmptySubscription.complete(subscriber);
                return true;
            }
            try {
                Publisher publisher2 = (Publisher) Objects.requireNonNull(function.apply(permission_groupVar), "The mapper returned a null Publisher");
