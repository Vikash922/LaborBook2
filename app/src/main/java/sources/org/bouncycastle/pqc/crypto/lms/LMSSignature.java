...
class LMSSignature implements Encodable {
    ...
    public boolean equals(Object obj) {
        ...
        if (this.f4635q != lMSSignature.f4635q) {
            return false;
        }
        ...
        if (lMSigParameters == null ? lMSSignature.parameter == null : lMSigParameters.equals(lMSSignature.parameter)) {
            return Arrays.deepEquals(this.f4636y, lMSSignature.f4636y);
        }
        return false;
    }
    ...
    public byte[] getEncoded() throws IOException {
        return Composer.compose().u32str(this.f4635q).bytes(this.otsSignature.getEncoded()).u32str(this.parameter.getType()).bytes(this.f4636y).build();
    }
    ...
    public int hashCode() {
        int i = this.f4635q * 31;
        LMOtsSignature lMOtsSignature = this.otsSignature;
        int iHashCode = (i + (lMOtsSignature != null ? lMOtsSignature.hashCode() : 0)) * 31;
        LMSigParameters lMSigParameters = this.parameter;
        return ((iHashCode + (lMSigParameters != null ? lMSigParameters.hashCode() : 0)) * 31) + Arrays.deepHashCode(this.f4636y);
    }
}
