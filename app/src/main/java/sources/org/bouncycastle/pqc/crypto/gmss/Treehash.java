...
public class Treehash {
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.tailLength + 6; i++) {
            string = string + getStatInt()[i] + StringUtils.SPACE;
        }
        for (int i2 = 0; i2 < this.tailLength + 3; i2++) {
            string = (getStatByte()[i2] != null ? new StringBuilder().append(string).append(new String(Hex.encode(getStatByte()[i2]))).append(StringUtils.SPACE) : new StringBuilder().append(string).append("null ")).toString();
        }
