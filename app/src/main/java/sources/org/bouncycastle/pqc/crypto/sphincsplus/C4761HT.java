...
/* JADX INFO: renamed from: org.bouncycastle.pqc.crypto.sphincsplus.HT */
...
class C4761HT {
    ...
    public C4761HT(SPHINCSPlusEngine sPHINCSPlusEngine, byte[] bArr, byte[] bArr2) {
        this.skSeed = bArr;
        this.pkSeed = bArr2;
        this.engine = sPHINCSPlusEngine;
        this.wots = new WotsPlus(sPHINCSPlusEngine);
        ADRS adrs = new ADRS();
        adrs.setLayerAddress(sPHINCSPlusEngine.f4703D - 1);
    ...
    byte[] sign(byte[] bArr, long j, int i) {
        ...
        for (int i3 = 1; i3 < this.engine.f4703D; i3++) {
            int i4 = (int) (((long) ((1 << this.engine.H_PRIME) - 1)) & j2);
            j2 >>>= this.engine.H_PRIME;
            adrs.setLayerAddress(i3);
            adrs.setTreeAddress(j2);
            SIG_XMSS sig_xmssXmss_sign2 = xmss_sign(bArrXmss_pkFromSig, this.skSeed, i4, this.pkSeed, adrs);
            sig_xmssArr[i3] = sig_xmssXmss_sign2;
            if (i3 < this.engine.f4703D - 1) {
                bArrXmss_pkFromSig = xmss_pkFromSig(i4, sig_xmssXmss_sign2, bArrXmss_pkFromSig, this.pkSeed, adrs);
            }
        }
    ...
    byte[] treehash(byte[] bArr, int i, int i2, byte[] bArr2, ADRS adrs) {
        ...
        for (int i4 = 0; i4 < i3; i4++) {
            ...
            while (!linkedList.isEmpty() && ((NodeEntry) linkedList.get(0)).nodeHeight == adrs2.getTreeHeight()) {
                ...
                bArrPkGen = this.engine.mo2889H(bArr2, adrs2, ((NodeEntry) linkedList.remove(0)).nodeValue, bArrPkGen);
    ...
    byte[] xmss_pkFromSig(int i, SIG_XMSS sig_xmss, byte[] bArr, byte[] bArr2, ADRS adrs) {
        ...
        while (i2 < this.engine.H_PRIME) {
            ...
            if ((i / (1 << i2)) % 2 == 0) {
                ...
                bArrPkFromSig = this.engine.mo2889H(bArr2, adrs2, bArrPkFromSig, xmssauth[i2]);
            } else {
                adrs2.setTreeIndex((adrs2.getTreeIndex() - 1) / 2);
                bArrPkFromSig = this.engine.mo2889H(bArr2, adrs2, xmssauth[i2], bArrPkFromSig);
            }
            i2 = i3;
        }
