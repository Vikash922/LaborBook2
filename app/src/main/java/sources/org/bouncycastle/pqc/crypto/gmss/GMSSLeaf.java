...
public class GMSSLeaf {
    ...
    public GMSSLeaf(Digest digest, byte[][] bArr, int[] iArr) {
        ...
        int digestSize = this.messDigestOTS.getDigestSize();
        this.mdsize = digestSize;
        int iCeil = (int) Math.ceil(((double) (digestSize << 3)) / ((double) this.f4606w));
        this.keysize = iCeil + ((int) Math.ceil(((double) getLog((iCeil << this.f4606w) + 1)) / ((double) this.f4606w)));
        this.two_power_w = 1 << this.f4606w;
    ...
    private GMSSLeaf(GMSSLeaf gMSSLeaf) {
        ...
        this.f4604i = gMSSLeaf.f4604i;
        this.f4605j = gMSSLeaf.f4605j;
        this.two_power_w = gMSSLeaf.two_power_w;
        this.f4606w = gMSSLeaf.f4606w;
    ...
    private void updateLeafCalc() {
        ...
        for (int i = 0; i < this.steps + 10000; i++) {
            int i2 = this.f4604i;
            if (i2 == this.keysize && this.f4605j == this.two_power_w - 1) {
            ...
            if (i2 == 0 || this.f4605j == this.two_power_w - 1) {
                this.f4604i = i2 + 1;
                this.f4605j = 0;
                ...
                int i3 = this.f4605j + 1;
                this.f4605j = i3;
                if (i3 == this.two_power_w - 1) {
                    byte[] bArr5 = this.privateKeyOTS;
                    byte[] bArr6 = this.concHashs;
                    int i4 = this.mdsize;
                    System.arraycopy(bArr5, 0, bArr6, (this.f4604i - 1) * i4, i4);
                }
            }
        }
        throw new IllegalStateException("unable to updateLeaf in steps: " + this.steps + StringUtils.SPACE + this.f4604i + StringUtils.SPACE + this.f4605j);
    }
    ...
    public int[] getStatInt() {
        return new int[]{this.f4604i, this.f4605j, this.steps, this.f4606w};
    }
    ...
    public String toString() {
        ...
        String string = str + StringUtils.SPACE + this.mdsize + StringUtils.SPACE + this.keysize + StringUtils.SPACE + this.two_power_w + StringUtils.SPACE;
        byte[][] statByte = getStatByte();
        for (int i2 = 0; i2 < 4; i2++) {
            string = (statByte[i2] != null ? new StringBuilder().append(string).append(new String(Hex.encode(statByte[i2]))).append(StringUtils.SPACE) : new StringBuilder().append(string).append("null ")).toString();
        }
        return string;
    }
}
