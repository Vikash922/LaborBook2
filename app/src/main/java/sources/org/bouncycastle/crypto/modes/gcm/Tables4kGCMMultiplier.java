...
public class Tables4kGCMMultiplier implements GCMMultiplier {
    ...
    public void init(byte[] bArr) {
        if (this.f4274T == null) {
            this.f4274T = (long[][]) Array.newInstance((Class<?>) Long.TYPE, 256, 2);
        } else if (GCMUtil.areEqual(this.f4273H, bArr) != 0) {
            return;
        }
        byte[] bArr2 = new byte[16];
        this.f4273H = bArr2;
        GCMUtil.copy(bArr, bArr2);
        GCMUtil.asLongs(this.f4273H, this.f4274T[1]);
        long[] jArr = this.f4274T[1];
        GCMUtil.multiplyP7(jArr, jArr);
        for (int i = 2; i < 256; i += 2) {
            long[][] jArr2 = this.f4274T;
            GCMUtil.divideP(jArr2[i >> 1], jArr2[i]);
            long[][] jArr3 = this.f4274T;
    ...
    public void multiplyH(byte[] bArr) {
        long[] jArr = this.f4274T[bArr[15] & 255];
        long j = jArr[0];
        long j2 = jArr[1];
        for (int i = 14; i >= 0; i--) {
            long[] jArr2 = this.f4274T[bArr[i] & 255];
            long j3 = j2 << 56;
