...
public class Tables64kGCMMultiplier implements GCMMultiplier {
    ...
    public void init(byte[] bArr) {
        if (this.f4276T == null) {
            this.f4276T = (long[][][]) Array.newInstance((Class<?>) Long.TYPE, 16, 256, 2);
        } else if (GCMUtil.areEqual(this.f4275H, bArr) != 0) {
            return;
        }
        ...
        for (int i = 0; i < 16; i++) {
            long[][][] jArr = this.f4276T;
            long[][] jArr2 = jArr[i];
            if (i == 0) {
                GCMUtil.asLongs(this.f4275H, jArr2[1]);
                long[] jArr3 = jArr2[1];
    ...
    public void multiplyH(byte[] bArr) {
        long[] jArr = this.f4276T[15][bArr[15] & 255];
        long j = jArr[0];
        long j2 = jArr[1];
        for (int i = 14; i >= 0; i--) {
            long[] jArr2 = this.f4276T[i][bArr[i] & 255];
            j ^= jArr2[0];
            j2 ^= jArr2[1];
        }
