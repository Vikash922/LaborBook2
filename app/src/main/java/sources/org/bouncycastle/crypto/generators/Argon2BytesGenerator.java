...
public class Argon2BytesGenerator {
    ...
    private static class Block {
        ...
        private final long[] f4178v;

        private Block() {
            this.f4178v = new long[128];
        }

        /* JADX INFO: Access modifiers changed from: private */
        public void copyBlock(Block block) {
            System.arraycopy(block.f4178v, 0, this.f4178v, 0, 128);
        }

        /* JADX INFO: Access modifiers changed from: private */
        public void xor(Block block, Block block2) {
            long[] jArr = this.f4178v;
            long[] jArr2 = block.f4178v;
            long[] jArr3 = block2.f4178v;
            for (int i = 0; i < 128; i++) {
        ...
        void fromBytes(byte[] bArr) {
            ...
            Pack.littleEndianToLong(bArr, 0, this.f4178v);
        }
        ...
        void toBytes(byte[] bArr) {
            ...
            Pack.longToLittleEndian(this.f4178v, bArr, 0);
        }
    }
    ...
    private static class FillBlock {
        ...
        private FillBlock() {
            this.f4179R = new Block();
            this.f4180Z = new Block();
        ...
        private void applyBlake() {
            for (int i = 0; i < 8; i++) {
                int i2 = i * 16;
                Argon2BytesGenerator.roundFunction(this.f4180Z, i2, i2 + 1, i2 + 2, i2 + 3, i2 + 4, i2 + 5, i2 + 6, i2 + 7, i2 + 8, i2 + 9, i2 + 10, i2 + 11, i2 + 12, i2 + 13, i2 + 14, i2 + 15);
            }
            for (int i3 = 0; i3 < 8; i3++) {
                int i4 = i3 * 2;
                Argon2BytesGenerator.roundFunction(this.f4180Z, i4, i4 + 1, i4 + 16, i4 + 17, i4 + 32, i4 + 33, i4 + 48, i4 + 49, i4 + 64, i4 + 65, i4 + 80, i4 + 81, i4 + 96, i4 + 97, i4 + 112, i4 + 113);
            }
        }

        /* JADX INFO: Access modifiers changed from: private */
        public void fillBlock(Block block, Block block2) {
            this.f4180Z.copyBlock(block);
            applyBlake();
            block2.xor(block, this.f4180Z);
        }

        /* JADX INFO: Access modifiers changed from: private */
        public void fillBlock(Block block, Block block2, Block block3) {
            this.f4179R.xor(block, block2);
            this.f4180Z.copyBlock(this.f4179R);
            applyBlake();
            block3.xor(this.f4179R, this.f4180Z);
        }
        ...
        public void fillBlockWithXor(Block block, Block block2, Block block3) {
            ...
            this.f4180Z.copyBlock(this.f4179R);
            applyBlake();
            block3.xorWith(this.f4179R, this.f4180Z);
        }
    }
    ...
    private static void m2864F(long[] jArr, int i, int i2, int i3, int i4) {
    ...
    private long getPseudoRandom(FillBlock fillBlock, int i, Block block, Block block2, int i2, boolean z) {
        if (!z) {
            return this.memory[i2].f4178v[0];
        }
        int i3 = i % 128;
        if (i3 == 0) {
            nextAddresses(fillBlock, block2, block);
        }
        return block.f4178v[i3];
    }
    ...
    private void initAddressBlocks(FillBlock fillBlock, Position position, Block block, Block block2) {
        block.f4178v[0] = intToLong(position.pass);
        block.f4178v[1] = intToLong(position.lane);
        block.f4178v[2] = intToLong(position.slice);
        block.f4178v[3] = intToLong(this.memory.length);
        block.f4178v[4] = intToLong(this.parameters.getIterations());
        block.f4178v[5] = intToLong(this.parameters.getType());
    ...
    public static void roundFunction(Block block, int i, int i2, int i3, int i4, int i5, int i6, int i7, int i8, int i9, int i10, int i11, int i12, int i13, int i14, int i15, int i16) {
        long[] jArr = block.f4178v;
        m2864F(jArr, i, i5, i9, i13);
        m2864F(jArr, i2, i6, i10, i14);
        m2864F(jArr, i3, i7, i11, i15);
        m2864F(jArr, i4, i8, i12, i16);
        m2864F(jArr, i, i6, i11, i16);
        m2864F(jArr, i2, i7, i12, i13);
        m2864F(jArr, i3, i8, i9, i14);
        m2864F(jArr, i4, i5, i10, i15);
    }
