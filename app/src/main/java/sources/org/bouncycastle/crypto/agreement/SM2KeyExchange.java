...
public class SM2KeyExchange {
    ...
    private byte[] m2769S1(Digest digest, ECPoint eCPoint, byte[] bArr) {
    ...
    private byte[] m2770S2(Digest digest, ECPoint eCPoint, byte[] bArr) {
    ...
    private BigInteger reduce(BigInteger bigInteger) {
        return bigInteger.and(BigInteger.valueOf(1L).shiftLeft(this.f3876w).subtract(BigInteger.valueOf(1L))).setBit(this.f3876w);
    }
    ...
    public byte[][] calculateKeyWithConfirmation(int i, byte[] bArr, CipherParameters cipherParameters) {
        ...
        if (!this.initiator) {
            ...
            return new byte[][]{bArrKdf, m2769S1(this.digest, eCPointCalculateU, bArrCalculateInnerHash), m2770S2(this.digest, eCPointCalculateU, bArrCalculateInnerHash)};
        }
        ...
        if (Arrays.constantTimeAreEqual(m2769S1(this.digest, eCPointCalculateU, bArrCalculateInnerHash2), bArr)) {
            return new byte[][]{bArrKdf2, m2770S2(this.digest, eCPointCalculateU, bArrCalculateInnerHash2)};
        }
    ...
    public void init(CipherParameters cipherParameters) {
        ...
        this.f3876w = (this.ecParams.getCurve().getFieldSize() / 2) - 1;
    }
}
