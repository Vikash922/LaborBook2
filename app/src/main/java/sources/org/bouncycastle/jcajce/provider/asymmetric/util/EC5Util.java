...
import org.bouncycastle.asn1.p031x9.ECNamedCurveTable;
...
public class EC5Util {
    ...
    static {
        ...
        customCurves.put(new ECCurve.C4651Fp(curve.getField().getCharacteristic(), curve.getA().toBigInteger(), curve.getB().toBigInteger(), curve.getOrder(), curve.getCofactor()), curve);
    }
    ...
    public static ECCurve convertCurve(EllipticCurve ellipticCurve) {
        ...
        if (field instanceof ECFieldFp) {
            ECCurve.C4651Fp c4651Fp = new ECCurve.C4651Fp(((ECFieldFp) field).getP(), a2, b);
            return customCurves.containsKey(c4651Fp) ? (ECCurve) customCurves.get(c4651Fp) : c4651Fp;
        }
        ECFieldF2m eCFieldF2m = (ECFieldF2m) field;
        int m = eCFieldF2m.getM();
        int[] iArrConvertMidTerms = ECUtil.convertMidTerms(eCFieldF2m.getMidTermsOfReductionPolynomial());
        return new ECCurve.F2m(m, iArrConvertMidTerms[0], iArrConvertMidTerms[1], iArrConvertMidTerms[2], a2, b);
    }
    ...
    public static ECPoint convertPoint(org.bouncycastle.math.p042ec.ECPoint eCPoint) {
        org.bouncycastle.math.p042ec.ECPoint eCPointNormalize = eCPoint.normalize();
        return new ECPoint(eCPointNormalize.getAffineXCoord().toBigInteger(), eCPointNormalize.getAffineYCoord().toBigInteger());
    }

    public static org.bouncycastle.math.p042ec.ECPoint convertPoint(ECParameterSpec eCParameterSpec, ECPoint eCPoint) {
        return convertPoint(convertCurve(eCParameterSpec.getCurve()), eCPoint);
    }

    public static org.bouncycastle.math.p042ec.ECPoint convertPoint(ECCurve eCCurve, ECPoint eCPoint) {
    ...
    public static org.bouncycastle.jce.spec.ECParameterSpec convertSpec(ECParameterSpec eCParameterSpec) {
        ...
        org.bouncycastle.math.p042ec.ECPoint eCPointConvertPoint = convertPoint(eCCurveConvertCurve, eCParameterSpec.getGenerator());
